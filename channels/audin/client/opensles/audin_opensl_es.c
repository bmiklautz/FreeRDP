...
static UINT audin_opensles_set_format(IAudinDevice* device, const AUDIO_FORMAT* format, UINT32 FramesPerPacket)
{
	...
	WLog_Print(opensles->log, WLOG_DEBUG, "device=%p, format=%p, FramesPerPacket=%" PRIu32 "", (void*) device,
	...
	WLog_Print(opensles->log, WLOG_DEBUG, "frames_per_packet=%" PRIu32, opensles->frames_per_packet);
	return CHANNEL_RC_OK;
}
...
static UINT audin_opensles_open(IAudinDevice* device, AudinReceive receive, void* user_data)
{
	...
	WLog_Print(opensles->log, WLOG_DEBUG, "device=%p, receive=%p, user_data=%p", (void*) device, (void*) receive,
	           (void*) user_data);

	if (opensles->stream)
		goto error_out;

	if (!(opensles->stream = android_OpenRecDevice(opensles, audin_receive, opensles->format.nSamplesPerSec,
	                                               opensles->format.nChannels, opensles->frames_per_packet,
...
static COMMAND_LINE_ARGUMENT_A audin_opensles_args[] = { { "dev", COMMAND_LINE_VALUE_REQUIRED, "<device>", NULL, NULL,
	                                                       -1, NULL, "audio device name" },
...
static UINT audin_opensles_parse_addin_args(AudinOpenSLESDevice* device, ADDIN_ARGV* args)
{
	...
	status = CommandLineParseArgumentsA(args->argc, args->argv, audin_opensles_args, flags, opensles, NULL, NULL);
...
UINT freerdp_audin_client_subsystem_entry(PFREERDP_AUDIN_DEVICE_ENTRY_POINTS pEntryPoints)
{
	...
	{
		WLog_Print(opensles->log, WLOG_ERROR, "audin_opensles_parse_addin_args failed with errorcode %" PRIu32 "!",
		           error);
		goto error_out;
	}
