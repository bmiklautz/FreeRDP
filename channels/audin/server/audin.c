...
static UINT audin_server_select_format(audin_server_context* context, size_t client_format_index)
{
	...
	if (!freerdp_dsp_context_reset(audin->dsp_context, &audin->context.client_formats[client_format_index]))
	{
...
static UINT audin_server_recv_version(audin_server* audin, wStream* s, UINT32 length)
{
	...
	if (length < 4)
	{
		WLog_ERR(TAG, "error parsing version info: expected at least 4 bytes, got %" PRIu32 "", length);
		return ERROR_INVALID_DATA;
	}
...
static UINT audin_server_send_formats(audin_server* audin, wStream* s)
{
	...
	return WTSVirtualChannelWrite(audin->audin_channel, (PCHAR) Stream_Buffer(s), Stream_GetPosition(s), &written)
	  ? CHANNEL_RC_OK
...
static UINT audin_server_recv_formats(audin_server* audin, wStream* s, UINT32 length)
{
	...
	if (length < 8)
	{
		WLog_ERR(TAG, "error parsing rec formats: expected at least 8 bytes, got %" PRIu32 "", length);
		return ERROR_INVALID_DATA;
	}
...
static UINT audin_server_send_open(audin_server* audin, wStream* s)
{
	...
	{
		WLog_ERR(TAG, "audin->context.selected_client_format = %d", audin->context.selected_client_format);
		return ERROR_INVALID_DATA;
	}
...
static DWORD WINAPI audin_server_thread_func(LPVOID arg)
{
	...
	while (ready)
	{
		...
		if (WTSVirtualChannelRead(audin->audin_channel, 0, (PCHAR) Stream_Buffer(s), Stream_Capacity(s),
		                          &BytesReturned) == FALSE)
		{
			WLog_ERR(TAG, "WTSVirtualChannelRead failed!");
			error = ERROR_INTERNAL_ERROR;
...
static BOOL audin_server_open(audin_server_context* context)
{
	...
	if (!audin->thread)
	{
		...
		audin->audin_channel = WTSVirtualChannelOpenEx(audin->SessionId, "AUDIO_INPUT", WTS_CHANNEL_OPTION_DYNAMIC);
