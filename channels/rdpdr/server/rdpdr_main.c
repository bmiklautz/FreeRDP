...
static void rdpdr_server_irp_free(RDPDR_IRP* irp) { free(irp); }
...
static UINT rdpdr_server_receive_client_name_request(RdpdrServerContext* context, wStream* s, RDPDR_HEADER* header)
{
	...
	if (Stream_Pointer(s)[ComputerNameLen - 1] || (UnicodeFlag && Stream_Pointer(s)[ComputerNameLen - 2]))
	{
	...
	if (UnicodeFlag)
	{
		if (ConvertFromUnicode(CP_UTF8, 0, (WCHAR*) Stream_Pointer(s), -1, &(context->priv->ClientComputerName), 0,
		                       NULL, NULL) < 1)
		{
...
static UINT rdpdr_server_send_core_capability_request(RdpdrServerContext* context)
{
	...
	{
		WLog_ERR(TAG, "rdpdr_server_write_general_capability_set failed with error %" PRIu32 "!", error);
		goto out;
	}
	...
	if (context->supportsDrives)
	{
		...
		{
			WLog_ERR(TAG, "rdpdr_server_write_drive_capability_set failed with error %" PRIu32 "!", error);
			goto out;
		}
	}
	...
	if (context->supportsPorts)
	{
		...
		{
			WLog_ERR(TAG, "rdpdr_server_write_port_capability_set failed with error %" PRIu32 "!", error);
			goto out;
		}
	}
	...
	if (context->supportsPrinters)
	{
		...
		{
			WLog_ERR(TAG, "rdpdr_server_write_printer_capability_set failed with error %" PRIu32 "!", error);
			goto out;
		}
	}
...
static UINT rdpdr_server_receive_core_capability_response(RdpdrServerContext* context, wStream* s, RDPDR_HEADER* header)
{
	...
	for (i = 0; i < numCapabilities; i++)
	{
		...
		{
			WLog_ERR(TAG, "rdpdr_server_read_capability_set_header failed with error %" PRIu32 "!", status);
			return status;
		}
		...
		switch (capabilityHeader.CapabilityType)
		{
			case CAP_GENERAL_TYPE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_read_general_capability_set failed with error %" PRIu32 "!", status);
					return status;
				}
			...
			case CAP_PRINTER_TYPE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_read_printer_capability_set failed with error %" PRIu32 "!", status);
					return status;
				}
			...
			case CAP_PORT_TYPE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_read_port_capability_set failed with error %" PRIu32 "!", status);
					return status;
				}
			...
			case CAP_DRIVE_TYPE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_read_drive_capability_set failed with error %" PRIu32 "!", status);
					return status;
				}
			...
			case CAP_SMARTCARD_TYPE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_read_smartcard_capability_set failed with error %" PRIu32 "!", status);
					return status;
				}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_receive_device_list_announce_request(RdpdrServerContext* context, wStream* s,
                                                              RDPDR_HEADER* header)
{
	int i;
	UINT32 DeviceCount;
	...
	for (i = 0; i < DeviceCount; i++)
	{
		...
		WLog_DBG(TAG, "Device %d Name: %s Id: 0x%08" PRIX32 " DataLength: %" PRIu32 "", i, PreferredDosName, DeviceId,
		         DeviceDataLength);
...
static UINT rdpdr_server_receive_device_io_completion(RdpdrServerContext* context, wStream* s, RDPDR_HEADER* header)
{
	...
	WLog_DBG(TAG, "deviceId=%" PRIu32 ", completionId=0x%" PRIx32 ", ioStatus=0x%" PRIx32 "", deviceId, completionId,
	         ioStatus);
...
{
	UINT error = CHANNEL_RC_OK;
	WLog_DBG(TAG, "RdpdrServerReceivePdu: Component: 0x%04" PRIX16 " PacketId: 0x%04" PRIX16 "", header->Component,
	         header->PacketId);
	winpr_HexDump(TAG, WLOG_DEBUG, Stream_Buffer(s), Stream_Length(s));
	...
	if (header->Component == RDPDR_CTYP_CORE)
	{
		switch (header->PacketId)
		{
			case PAKID_CORE_CLIENTID_CONFIRM:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_receive_announce_response failed with error %" PRIu32 "!", error);
					return error;
				}
			...
			case PAKID_CORE_CLIENT_NAME:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_receive_client_name_request failed with error %" PRIu32 "!", error);
					return error;
				}

				if ((error = rdpdr_server_send_core_capability_request(context)))
				{
					WLog_ERR(TAG, "rdpdr_server_send_core_capability_request failed with error %" PRIu32 "!", error);
					return error;
				}

				if ((error = rdpdr_server_send_client_id_confirm(context)))
				{
					WLog_ERR(TAG, "rdpdr_server_send_client_id_confirm failed with error %" PRIu32 "!", error);
					return error;
				}
			...
			case PAKID_CORE_CLIENT_CAPABILITY:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_receive_core_capability_response failed with error %" PRIu32 "!",
					         error);
					return error;
				}
			...
			case PAKID_CORE_DEVICELIST_ANNOUNCE:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_receive_device_list_announce_request failed with error %" PRIu32 "!",
					         error);
					return error;
				}
			...
			case PAKID_CORE_DEVICE_IOCOMPLETION:
				...
				{
					WLog_ERR(TAG, "rdpdr_server_receive_device_io_completion failed with error %" PRIu32 "!", error);
					return error;
				}
...
static UINT rdpdr_server_start(RdpdrServerContext* context)
{
	...
	if (!(context->priv->Thread = CreateThread(NULL, 0, rdpdr_server_thread, (void*) context, 0, NULL)))
	{
...
static void rdpdr_server_write_device_iorequest(wStream* s, UINT32 deviceId, UINT32 fileId, UINT32 completionId,
                                                UINT32 majorFunction, UINT32 minorFunction)
{
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_create_request(RdpdrServerContext* context, UINT32 deviceId, UINT32 completionId,
                                                    const char* path, UINT32 desiredAccess, UINT32 createOptions,
                                                    UINT32 createDisposition)
{
	...
	wStream* s;
	WLog_DBG(TAG,
	         "RdpdrServerSendDeviceCreateRequest: deviceId=%" PRIu32 ", path=%s, desiredAccess=0x%" PRIx32
	         " createOptions=0x%" PRIx32 " createDisposition=0x%" PRIx32 "",
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_close_request(RdpdrServerContext* context, UINT32 deviceId, UINT32 fileId,
                                                   UINT32 completionId)
{
	ULONG written;
	BOOL status;
	wStream* s;
	WLog_DBG(TAG, "RdpdrServerSendDeviceCloseRequest: deviceId=%" PRIu32 ", fileId=%" PRIu32 "", deviceId, fileId);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_read_request(RdpdrServerContext* context, UINT32 deviceId, UINT32 fileId,
                                                  UINT32 completionId, UINT32 length, UINT32 offset)
{
	ULONG written;
	BOOL status;
	wStream* s;
	WLog_DBG(TAG,
	         "RdpdrServerSendDeviceReadRequest: deviceId=%" PRIu32 ", fileId=%" PRIu32 ", length=%" PRIu32
	         ", offset=%" PRIu32 "",
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_write_request(RdpdrServerContext* context, UINT32 deviceId, UINT32 fileId,
                                                   UINT32 completionId, const char* data, UINT32 length, UINT32 offset)
{
	ULONG written;
	BOOL status;
	wStream* s;
	WLog_DBG(TAG,
	         "RdpdrServerSendDeviceWriteRequest: deviceId=%" PRIu32 ", fileId=%" PRIu32 ", length=%" PRIu32
	         ", offset=%" PRIu32 "",
	         deviceId, fileId, length, offset);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_query_directory_request(RdpdrServerContext* context, UINT32 deviceId,
                                                             UINT32 fileId, UINT32 completionId, const char* path)
{
	UINT32 pathLength;
	ULONG written;
	BOOL status;
	wStream* s;
	WLog_DBG(TAG, "RdpdrServerSendDeviceQueryDirectoryRequest: deviceId=%" PRIu32 ", fileId=%" PRIu32 ", path=%s",
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_send_device_file_rename_request(RdpdrServerContext* context, UINT32 deviceId, UINT32 fileId,
                                                         UINT32 completionId, const char* path)
{
	...
	BOOL status;
	wStream* s;
	WLog_DBG(TAG, "RdpdrServerSendDeviceFileNameRequest: deviceId=%" PRIu32 ", fileId=%" PRIu32 ", path=%s", deviceId,
	         fileId, path);
	/* Compute the required Unicode size. */
	pathLength = path ? (strlen(path) + 1) * sizeof(WCHAR) : 0;
	s = Stream_New(NULL, 64 + pathLength);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_create_directory_callback2(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_create_directory_callback1(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                          UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	UINT8 information;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_create_directory(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                                const char* path)
{
	RDPDR_IRP* irp;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_directory_callback2(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                          UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	WLog_DBG(TAG,
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_directory_callback1(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                          UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	UINT8 information;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_directory(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                                const char* path)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_query_directory_callback2(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                         UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT error;
	UINT32 length;
	...
	if (length > 0)
	{
		...
		{
			WLog_ERR(TAG, "rdpdr_server_read_file_directory_information failed with error %" PRIu32 "!", error);
			return error;
		}
	}
	else
	...
	if (ioStatus == STATUS_SUCCESS)
	{
		...
		return rdpdr_server_send_device_query_directory_request(context, irp->DeviceId, irp->FileId, irp->CompletionId,
		                                                        NULL);
	}
	else
	{
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_query_directory_callback1(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                         UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	WLog_DBG(TAG,
	...
	/* Send a request to query the directory. */
	return rdpdr_server_send_device_query_directory_request(context, deviceId, fileId, irp->CompletionId,
	                                                        irp->PathName);
}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_query_directory(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                               const char* path)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_open_file_callback(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                  UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	UINT8 information;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_open_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                         const char* path, UINT32 desiredAccess, UINT32 createDisposition)
{
	...
	/* Send a request to open the file. */
	return rdpdr_server_send_device_create_request(context, deviceId, irp->CompletionId, irp->PathName,
	                                               desiredAccess | SYNCHRONIZE, FILE_SYNCHRONOUS_IO_NONALERT,
	                                               createDisposition);
}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_read_file_callback(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                  UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 length;
	char* buffer = NULL;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_read_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                         UINT32 fileId, UINT32 length, UINT32 offset)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_write_file_callback(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                   UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 length;
	WLog_DBG(
	  TAG, "RdpdrServerDriveWriteFileCallback: deviceId=%" PRIu32 ", completionId=%" PRIu32 ", ioStatus=0x%" PRIx32 "",
	  deviceId, completionId, ioStatus);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_write_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                          UINT32 fileId, const char* buffer, UINT32 length, UINT32 offset)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_close_file_callback(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                   UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	WLog_DBG(
	  TAG, "RdpdrServerDriveCloseFileCallback: deviceId=%" PRIu32 ", completionId=%" PRIu32 ", ioStatus=0x%" PRIx32 "",
	  deviceId, completionId, ioStatus);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_close_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                          UINT32 fileId)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_file_callback2(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                     UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	WLog_DBG(TAG,
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_file_callback1(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                     UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	UINT8 information;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_delete_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                           const char* path)
{
	RDPDR_IRP* irp;
	irp = rdpdr_server_irp_new();
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_rename_file_callback3(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                     UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	WLog_DBG(TAG,
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_rename_file_callback2(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                     UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 length;
	WLog_DBG(TAG,
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_rename_file_callback1(RdpdrServerContext* context, wStream* s, RDPDR_IRP* irp,
                                                     UINT32 deviceId, UINT32 completionId, UINT32 ioStatus)
{
	UINT32 fileId;
	UINT8 information;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT rdpdr_server_drive_rename_file(RdpdrServerContext* context, void* callbackData, UINT32 deviceId,
                                           const char* oldPath, const char* newPath)
{
	...
	/* Send a request to open the file. */
	return rdpdr_server_send_device_create_request(context, deviceId, irp->CompletionId, irp->PathName,
	                                               FILE_READ_DATA | SYNCHRONIZE, FILE_SYNCHRONOUS_IO_NONALERT,
	                                               FILE_OPEN);
}
