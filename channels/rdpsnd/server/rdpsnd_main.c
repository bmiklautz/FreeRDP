...
static UINT rdpsnd_server_send_formats(RdpsndServerContext* context, wStream* s)
{
	...
	status =
	  WTSVirtualChannelWrite(context->priv->ChannelHandle, (PCHAR) Stream_Buffer(s), Stream_GetPosition(s), &written);
...
static UINT rdpsnd_server_send_wave_pdu(RdpsndServerContext* context, UINT16 wTimestamp)
{
	...
	else
	{
		...
		if (!WTSVirtualChannelWrite(context->priv->ChannelHandle, (PCHAR) Stream_Buffer(s), start + 4, &written))
		{
	...
	if (!WTSVirtualChannelWrite(context->priv->ChannelHandle, (PCHAR) Stream_Pointer(s), end - start, &written))
	{
		WLog_ERR(TAG, "WTSVirtualChannelWrite failed!");
		error = ERROR_INTERNAL_ERROR;
	}
...
static UINT rdpsnd_server_send_wave2_pdu(RdpsndServerContext* context, UINT16 wTimestamp)
{
	...
	else
	{
		...
		context->block_no = (context->block_no + 1) % 256;
		rc = WTSVirtualChannelWrite(context->priv->ChannelHandle, (PCHAR) Stream_Buffer(s), end, &written);

		if (!rc || (end != written))
		{
			WLog_ERR(TAG, "WTSVirtualChannelWrite failed! [stream length=%" PRIdz " - written=%" PRIu32, end, written);
			error = ERROR_INTERNAL_ERROR;
		}
	}
...
static UINT rdpsnd_server_send_samples(RdpsndServerContext* context, const void* buf, int nframes, UINT16 wTimestamp)
{
	...
	while (nframes > 0)
	{
		...
		CopyMemory(context->priv->out_buffer + (context->priv->out_pending_frames * context->priv->src_bytes_per_frame),
		           buf, cframesize);
...
static UINT rdpsnd_server_start(RdpsndServerContext* context)
{
	...
	if (priv->ownThread)
	{
		...
		context->priv->Thread = CreateThread(NULL, 0, rdpsnd_server_thread, (void*) context, 0, NULL);
...
HANDLE rdpsnd_server_get_event_handle(RdpsndServerContext* context) { return context->priv->channelEvent; }
