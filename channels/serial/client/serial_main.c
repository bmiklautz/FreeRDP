...
static UINT serial_process_irp_create(SERIAL_DEVICE* serial, IRP* irp)
{
	 ...
	 *     DesiredAccess     = 0x00120089: SYNCHRONIZE | READ_CONTROL | FILE_READ_ATTRIBUTES | FILE_READ_EA |
	 * FILE_READ_DATA SharedAccess      = 0x00000007: FILE_SHARE_DELETE | FILE_SHARE_WRITE | FILE_SHARE_READ
	 *     CreateDisposition = 0x00000001: CREATE_NEW
	 *
...
#endif
	serial->hComm = CreateFile(serial->device.name, DesiredAccess, SharedAccess, NULL, /* SecurityAttributes */
	                           CreateDisposition, 0, /* FlagsAndAttributes */
	                           NULL); /* TemplateFile */

	if (!serial->hComm || (serial->hComm == INVALID_HANDLE_VALUE))
	{
		WLog_Print(serial->log, WLOG_WARN, "CreateFile failure: %s last-error: 0x%08" PRIX32 "", serial->device.name,
		           GetLastError());
	...
	WLog_Print(serial->log, WLOG_DEBUG, "%s (DeviceId: %" PRIu32 ", FileId: %" PRIu32 ") created.", serial->device.name,
	           irp->device->id, irp->FileId);
error_handle:
...
static UINT serial_process_irp_close(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	{
		WLog_Print(serial->log, WLOG_WARN, "CloseHandle failure: %s (%" PRIu32 ") closed.", serial->device.name,
		           irp->device->id);
		irp->IoStatus = STATUS_UNSUCCESSFUL;
		goto error_handle;
	}

	WLog_Print(serial->log, WLOG_DEBUG, "%s (DeviceId: %" PRIu32 ", FileId: %" PRIu32 ") closed.", serial->device.name,
	           irp->device->id, irp->FileId);
	serial->hComm = NULL;
	irp->IoStatus = STATUS_SUCCESS;
error_handle:
...
static UINT serial_process_irp_read(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	{
		WLog_Print(serial->log, WLOG_DEBUG, "read failure to %s, nbRead=%" PRIu32 ", last-error: 0x%08" PRIX32 "",
...
static UINT serial_process_irp_write(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	{
		WLog_Print(serial->log, WLOG_DEBUG, "write failure to %s, nbWritten=%" PRIu32 ", last-error: 0x%08" PRIX32 "",
...
static UINT serial_process_irp_device_control(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	WLog_Print(serial->log, WLOG_DEBUG,
	           "CommDeviceIoControl: CompletionId=%" PRIu32 ", IoControlCode=[0x%" PRIX32 "] %s", irp->CompletionId,
	           IoControlCode, _comm_serial_ioctl_name(IoControlCode));

	/* FIXME: CommDeviceIoControl to be replaced by DeviceIoControl() */
	if (CommDeviceIoControl(serial->hComm, IoControlCode, InputBuffer, InputBufferLength, OutputBuffer,
	                        OutputBufferLength, &BytesReturned, NULL))
	{
		/* WLog_Print(serial->log, WLOG_DEBUG, "CommDeviceIoControl: CompletionId=%"PRIu32", IoControlCode=[0x%"PRIX32"]
		 * %s done", irp->CompletionId, IoControlCode, _comm_serial_ioctl_name(IoControlCode)); */
		irp->IoStatus = STATUS_SUCCESS;
	}
	else
	{
...
static void create_irp_thread(SERIAL_DEVICE* serial, IRP* irp)
{
	...
	if (previousIrpThread)
	{
		...
		WLog_Print(serial->log, WLOG_DEBUG, "IRP recall: IRP with the CompletionId=%" PRIu32 " not yet completed!",
		           irp->CompletionId);
	...
	{
		WLog_Print(serial->log, WLOG_WARN, "Number of IRP threads threshold reached: %d, keep on anyway",
