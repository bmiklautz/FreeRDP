...
UINT tsmf_ifman_set_source_video_rect(TSMF_IFMAN* ifman)
{
	...
	else
	{
		...
		DEBUG_TSMF("SetSourceVideoRect: Left: %f Top: %f Right: %f Bottom: %f", Left, Top, Right, Bottom);
	}
...
UINT tsmf_ifman_update_geometry_info(TSMF_IFMAN* ifman)
{
	...
	DEBUG_TSMF("numGeometryInfo %" PRIu32 " Width %" PRIu32 " Height %" PRIu32 " Left %" PRIu32 " Top %" PRIu32
	           " cbVisibleRect %" PRIu32 " num_rects %d",
	...
	if (num_rects > 0)
	{
		...
		for (i = 0; i < num_rects; i++)
		{
			...
			DEBUG_TSMF("rect %d: %" PRId16 " %" PRId16 " %" PRId16 " %" PRId16 "", i, rects[i].x, rects[i].y,
...
UINT tsmf_ifman_on_sample(TSMF_IFMAN* ifman)
{
	...
	DEBUG_TSMF("MessageId %" PRIu32 " StreamId %" PRIu32 " SampleStartTime %" PRIu64 " SampleEndTime %" PRIu64 " "
	           ...
	           ifman->message_id, StreamId, SampleStartTime, SampleEndTime, ThrottleDuration, SampleExtensions, cbData);
	...
	if (!tsmf_stream_push_sample(stream, ifman->channel_callback, ifman->message_id, SampleStartTime, SampleEndTime,
