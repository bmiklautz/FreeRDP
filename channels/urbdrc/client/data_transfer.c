...
static int urbdrc_process_register_request_callback(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
...
static int urbdrc_process_io_control(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
                                     IUDEVMAN* udevman, UINT32 UsbDevice)
{
	IUDEVICE* pdev;
	UINT32 out_size;
	...
	switch (IoControlCode)
	{
		...
		case IOCTL_INTERNAL_USB_GET_PORT_STATUS: /** 0x00220013 */
			...
			if (success)
			{
				...
				WLog_DBG(TAG, "PORT STATUS(fake!):0x%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "", OutputBuffer[3],
				         OutputBuffer[2], OutputBuffer[1], OutputBuffer[0]);
			}
...
static int urbdrc_process_internal_io_control(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
...
static int urbdrc_process_query_device_text(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                            UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice)
{
	IUDEVICE* pdev;
	UINT32 out_size;
...
static int urb_select_configuration(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
...
static int urb_select_interface(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
                                IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
...
static int urb_control_transfer(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
                                IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir, int External)
{
	IUDEVICE* pdev;
	...
	ret = pdev->control_transfer(pdev, RequestId, EndpointAddress, TransferFlags, bmRequestType, Request, Value, Index,
	                             &usbd_status, &OutputBufferSize, buffer, Timeout);
...
static int urb_bulk_or_interrupt_transfer(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                          UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	...
	pdev->bulk_or_interrupt_transfer(pdev, RequestId, EndpointAddress, TransferFlags, &usbd_status, &OutputBufferSize,
	                                 Buffer, 10000);
...
static int urb_isoch_transfer(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
                              IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
	...
	WLog_DBG(TAG,
	         ...
	         "RequestId: 0x%" PRIx32 "",
	         EndpointAddress, TransferFlags, StartFrame, NumberOfPackets, OutputBufferSize, RequestId);
	...
	iso_status = pdev->isoch_transfer(pdev, RequestId, EndpointAddress, TransferFlags, noAck, &ErrorCount, &usbd_status,
	                                  &StartFrame, NumberOfPackets, iso_packets, &OutputBufferSize, iso_buffer, 2000);
...
static int urb_control_descriptor_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                          UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, BYTE func_recipient,
                                          int transferDir)
{
	IUDEVICE* pdev;
	...
	ret = pdev->control_transfer(pdev, RequestId, 0, 0, bmRequestType, 0x06, /* REQUEST_GET_DESCRIPTOR */
...
static int urb_control_get_status_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                          UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, BYTE func_recipient,
                                          int transferDir)
{
	IUDEVICE* pdev;
	...
	ret = pdev->control_transfer(pdev, RequestId, 0, 0, bmRequestType, 0x00, /* REQUEST_GET_STATUS */
...
static int urb_control_vendor_or_class_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                               UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, BYTE func_type,
                                               BYTE func_recipient, int transferDir)
{
	...
	WLog_DBG(TAG,
	         ...
	         "Request:0x%" PRIx8 " Value: 0x%" PRIx16 " Index: 0x%" PRIx16 " OutputBufferSize: 0x%" PRIx32
	         " bmRequestType: 0x%" PRIx8 "!!",
	         RequestId, TransferFlags, ReqTypeReservedBits, Request, Value, Index, OutputBufferSize, bmRequestType);

	ret = pdev->control_transfer(pdev, RequestId, 0, 0, bmRequestType, Request, Value, Index, &usbd_status,
	                             &OutputBufferSize, buffer, 2000);
...
static int urb_os_feature_descriptor_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                             UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
	UINT32 out_size, RequestId, InterfaceId, OutputBufferSize, usbd_status;
	...
	/** get ms string */
	ret = pdev->os_feature_descriptor_request(pdev, RequestId, Recipient, InterfaceNumber, Ms_PageIndex,
	                                          Ms_featureDescIndex, &usbd_status, &OutputBufferSize, buffer, 1000);
...
static int urb_pipe_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem, UINT32 MessageId,
                            IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir, int action)
{
	IUDEVICE* pdev;
	...
	switch (action)
	{
		case PIPE_CANCEL:
			...
			ret = pdev->control_pipe_request(pdev, RequestId, EndpointAddress, &usbd_status, PIPE_CANCEL);
		...
		case PIPE_RESET:
			...
			ret = pdev->control_pipe_request(pdev, RequestId, EndpointAddress, &usbd_status, PIPE_RESET);
...
static int urb_get_current_frame_number(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                        UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
...
static int urb_control_get_configuration_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                                 UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
	UINT32 out_size, RequestId, InterfaceId, OutputBufferSize, usbd_status;
	...
	ret = pdev->control_transfer(pdev, RequestId, 0, 0, 0x80 | 0x00, 0x08, /* REQUEST_GET_CONFIGURATION */
...
/* Unused function for current server */
static int urb_control_get_interface_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                             UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
	UINT32 out_size, RequestId, InterfaceId, OutputBufferSize, usbd_status;
	...
	ret = pdev->control_transfer(pdev, RequestId, 0, 0, 0x80 | 0x01, 0x0A, /* REQUEST_GET_INTERFACE */
...
static int urb_control_feature_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                       UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, BYTE func_recipient,
                                       BYTE command, int transferDir)
{
	IUDEVICE* pdev;
	UINT32 out_size, RequestId, InterfaceId, OutputBufferSize, usbd_status;
	...
	ret = pdev->control_transfer(pdev, RequestId, 0, 0, bmRequestType, bmRequest, FeatureSelector, Index, &usbd_status,
...
static int urbdrc_process_transfer_request(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data, UINT32 data_sizem,
                                           UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	IUDEVICE* pdev;
	UINT32 CbTsUrb;
	...
	switch (URB_Function)
	{
		case URB_FUNCTION_SELECT_CONFIGURATION: /** 0x0000 */
			...
			error =
			  urb_select_configuration(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, transferDir);
			break;
		case URB_FUNCTION_SELECT_INTERFACE: /** 0x0001 */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_SELECT_INTERFACE");
			error =
			  urb_select_interface(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, transferDir);
			break;
		case URB_FUNCTION_ABORT_PIPE: /** 0x0002  */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_ABORT_PIPE");
			error = urb_pipe_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, transferDir,
			                         PIPE_CANCEL);
			break;
		...
		case URB_FUNCTION_GET_CURRENT_FRAME_NUMBER: /** 0x0007 */
			...
			error = urb_get_current_frame_number(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                     transferDir);
			break;
		case URB_FUNCTION_CONTROL_TRANSFER: /** 0x0008 */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_CONTROL_TRANSFER");
			error = urb_control_transfer(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, transferDir,
		...
		case URB_FUNCTION_BULK_OR_INTERRUPT_TRANSFER: /** 0x0009 */
			...
			error = urb_bulk_or_interrupt_transfer(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                       transferDir);
			break;
		case URB_FUNCTION_ISOCH_TRANSFER: /** 0x000A */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_ISOCH_TRANSFER");
			error = urb_isoch_transfer(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, transferDir);
			break;
		case URB_FUNCTION_GET_DESCRIPTOR_FROM_DEVICE: /** 0x000B */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_GET_DESCRIPTOR_FROM_DEVICE");
			error = urb_control_descriptor_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                       0x00, transferDir);
			break;
		...
		case URB_FUNCTION_SET_FEATURE_TO_DEVICE: /** 0x000D */
			...
			error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, 0x00,
			                                    URB_SET_FEATURE, transferDir);
			break;
		case URB_FUNCTION_SET_FEATURE_TO_INTERFACE: /** 0x000E */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_SET_FEATURE_TO_INTERFACE");
			error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, 0x01,
			                                    URB_SET_FEATURE, transferDir);
			break;
		case URB_FUNCTION_SET_FEATURE_TO_ENDPOINT: /** 0x000F */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_SET_FEATURE_TO_ENDPOINT");
			error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, 0x02,
			                                    URB_SET_FEATURE, transferDir);
			break;
		case URB_FUNCTION_CLEAR_FEATURE_TO_DEVICE: /** 0x0010 */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_CLEAR_FEATURE_TO_DEVICE");
			error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, 0x00,
			                                    URB_CLEAR_FEATURE, transferDir);
			break;
		...
		case URB_FUNCTION_GET_STATUS_FROM_DEVICE: /** 0x0013 */
			...
			error = urb_control_get_status_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                       0x00, transferDir);
			break;
		...
		case URB_FUNCTION_VENDOR_DEVICE: /** 0x0017 */
			...
			error = urb_control_vendor_or_class_request(callback, data + 8, data_sizem - 8, MessageId, udevman,
			                                            UsbDevice, (0x02 << 5), /* vendor type */
			                                            0x00, transferDir);
			break;
		...
		case URB_FUNCTION_CLEAR_FEATURE_TO_OTHER: /** 0x0022 */
			...
			error = urb_control_feature_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice, 0x03,
			                                    URB_CLEAR_FEATURE, transferDir);
			break;
		...
		case URB_FUNCTION_GET_CONFIGURATION: /** 0x0026 */
			...
			error = urb_control_get_configuration_request(callback, data + 8, data_sizem - 8, MessageId, udevman,
			                                              UsbDevice, transferDir);
			break;
		case URB_FUNCTION_GET_INTERFACE: /** 0x0027 */
			WLog_DBG(TAG, "URB_Func: URB_FUNCTION_GET_INTERFACE");
			error = urb_control_get_interface_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                          transferDir);
			break;
		...
		case URB_FUNCTION_GET_MS_FEATURE_DESCRIPTOR: /** 0x002A */
			...
			error = urb_os_feature_descriptor_request(callback, data + 8, data_sizem - 8, MessageId, udevman, UsbDevice,
			                                          transferDir);
			break;
	...
	switch (FunctionId)
	{
		...
		case REGISTER_REQUEST_CALLBACK:
			WLog_DBG(TAG,
			         ...
			         FunctionId);
			error = urbdrc_process_register_request_callback(callback, pBuffer + 8, cbSize - 8, udevman, UsbDevice);
			break;
		case IO_CONTROL:
			WLog_DBG(TAG,
			         "urbdrc_process_udev_data_transfer:"
			         ...
			         FunctionId);
			error = urbdrc_process_io_control(callback, pBuffer + 8, cbSize - 8, MessageId, udevman, UsbDevice);
			break;
		case INTERNAL_IO_CONTROL:
			WLog_DBG(TAG,
			         ...
			         FunctionId);
			error =
			  urbdrc_process_internal_io_control(callback, pBuffer + 8, cbSize - 8, MessageId, udevman, UsbDevice);
			break;
		case QUERY_DEVICE_TEXT:
			WLog_DBG(TAG,
			         ...
			         FunctionId);
			error = urbdrc_process_query_device_text(callback, pBuffer + 8, cbSize - 8, MessageId, udevman, UsbDevice);
			break;
		case TRANSFER_IN_REQUEST:
			WLog_DBG(TAG,
			         ...
			         FunctionId);
			error = urbdrc_process_transfer_request(callback, pBuffer + 8, cbSize - 8, MessageId, udevman, UsbDevice,
