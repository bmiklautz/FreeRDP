...
static void func_iso_data_init(ISO_USER_DATA* iso_user_data, UINT32 numPacket, UINT32 buffsize, UINT32 noAck,
                               BYTE* isoPacket, BYTE* buffer)
{
...
static int libusb_udev_select_interface(IUDEVICE* idev, BYTE InterfaceNumber, BYTE AlternateSetting)
{
	...
	if (diff)
	{
		error = libusb_set_interface_alt_setting(pdev->libusb_handle, InterfaceNumber, AlternateSetting);
...
static MSUSB_CONFIG_DESCRIPTOR* libusb_udev_complete_msconfig_setup(IUDEVICE* idev, MSUSB_CONFIG_DESCRIPTOR* MsConfig)
{
	 ...
	 * ---------------------------------------------------------------
	 * ***********************/
	MsConfig->ConfigurationHandle = MsConfig->bConfigurationValue | (pdev->bus_number << 24) | (pdev->dev_number << 16);
	MsInterfaces = MsConfig->MsInterfaces;
	...
	for (inum = 0; inum < MsConfig->NumInterfaces; inum++)
	{
		 ...
		 * ---------------------------------------------------------------
		 * ***********************/
		MsInterface->InterfaceHandle = LibusbAltsetting->bInterfaceNumber | (LibusbAltsetting->bAlternateSetting << 8) |
		...
		for (pnum = 0; pnum < LibusbNumEndpoint; pnum++)
		{
			 ...
			 * ---------------------------------------------------------------
			 * ***********************/
			MsPipe->PipeHandle = LibusbEndpoint->bEndpointAddress | (pdev->dev_number << 16) | (pdev->bus_number << 24);
...
static int libusb_udev_control_pipe_request(IUDEVICE* idev, UINT32 RequestId, UINT32 EndpointAddress,
	...
	*/
	switch (command)
	{
		case PIPE_CANCEL:
			...
			error = libusb_control_transfer(pdev->libusb_handle, LIBUSB_ENDPOINT_OUT | LIBUSB_RECIPIENT_ENDPOINT,
...
static int libusb_udev_control_query_device_text(IUDEVICE* idev, UINT32 TextType, UINT32 LocaleId, UINT32* BufferSize,
                                                 BYTE* Buffer)
{
	UDEVICE* pdev = (UDEVICE*) idev;
	...
	switch (TextType)
	{
		case DeviceTextDescription:
			ret =
			  libusb_get_string_descriptor(pdev->libusb_handle, devDescriptor->iProduct, LocaleId, Buffer, *BufferSize);
		...
		case DeviceTextLocationInformation:
			...
			sprintf_s(deviceLocation, ARRAYSIZE(deviceLocation), "Port_#%04" PRIu8 ".Hub_#%04" PRIu8 "", device_address,
			          bus_number);
...
static int libusb_udev_os_feature_descriptor_request(IUDEVICE* idev, UINT32 RequestId, BYTE Recipient,
                                                     BYTE InterfaceNumber, BYTE Ms_PageIndex,
                                                     UINT16 Ms_featureDescIndex, UINT32* UsbdStatus, UINT32* BufferSize,
                                                     BYTE* Buffer, int Timeout)
{
	...
	*/
	...
	error = libusb_control_transfer(pdev->libusb_handle, LIBUSB_ENDPOINT_IN | Recipient, LIBUSB_REQUEST_GET_DESCRIPTOR,
	                                0x03ee, 0, ms_string_desc, 0x12, Timeout);
	...
	if (error > 0)
	{
		...
		error = libusb_control_transfer(
		  pdev->libusb_handle, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_VENDOR | Recipient, bMS_Vendorcode,
		  (InterfaceNumber << 8) | Ms_PageIndex, Ms_featureDescIndex, Buffer, *BufferSize, Timeout);
		*BufferSize = error;
	}
...
static int libusb_udev_query_device_port_status(IUDEVICE* idev, UINT32* UsbdStatus, UINT32* BufferSize, BYTE* Buffer)
{
	...
	if (pdev->hub_handle != NULL)
	{
		ret = idev->control_transfer(
		  idev, 0xffff, 0, 0, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_OTHER,
		...
		{
			WLog_DBG(TAG, "PORT STATUS:0x%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "", Buffer[3], Buffer[2],
			         Buffer[1], Buffer[0]);
			success = 1;
		}
	}
...
static int libusb_udev_isoch_transfer(IUDEVICE* idev, UINT32 RequestId, UINT32 EndpointAddress, UINT32 TransferFlags,
                                      int NoAck, UINT32* ErrorCount, UINT32* UrbdStatus, UINT32* StartFrame,
                                      UINT32 NumberOfPackets, BYTE* IsoPacket, UINT32* BufferSize, BYTE* Buffer,
                                      int Timeout)
{
	...
	/** fill setting */
	libusb_fill_iso_transfer(iso_transfer, pdev->libusb_handle, EndpointAddress, Buffer, *BufferSize, NumberOfPackets,
	                         func_iso_callback, &iso_user_data, 2000);
...
static int libusb_udev_control_transfer(IUDEVICE* idev, UINT32 RequestId, UINT32 EndpointAddress, UINT32 TransferFlags,
                                        BYTE bmRequestType, BYTE Request, UINT16 Value, UINT16 Index,
                                        UINT32* UrbdStatus, UINT32* BufferSize, BYTE* Buffer, UINT32 Timeout)
{
	int status = 0;
	UDEVICE* pdev = (UDEVICE*) idev;
	status =
	  libusb_control_transfer(pdev->libusb_handle, bmRequestType, Request, Value, Index, Buffer, *BufferSize, Timeout);
...
static int libusb_udev_bulk_or_interrupt_transfer(IUDEVICE* idev, UINT32 RequestId, UINT32 EndpointAddress,
                                                  ...
                                                  BYTE* Buffer, UINT32 Timeout)
{
	...
	switch (transfer_type)
	{
		...
		case INTERRUPT_TRANSFER:
			...
			if (*BufferSize != (ep_desc->wMaxPacketSize))
			{
				...
				if ((*BufferSize) > (ep_desc->wMaxPacketSize) && transferDir == USBD_TRANSFER_DIRECTION_IN)
	...
	{
		request = pdev->request_queue->register_request(pdev->request_queue, RequestId, transfer, EndpointAddress);
		request->submit = 1;
	}

	if ((pdev && *UsbdStatus == 0) && (submit >= 0) && (pdev->iface.isSigToEnd((IUDEVICE*) pdev) == 0))
	{
		while (!completed)
		{
...
static void libusb_udev_cancel_all_transfer_request(IUDEVICE* idev)
{
	...
	while (request_queue->has_next(request_queue))
	{
		...
		if ((!request) || (!request->transfer) || (request->endpoint != request->transfer->endpoint) ||
...
cancel_retry:
	...
	while (request_queue->has_next(request_queue))
	{
		...
		WLog_DBG(TAG, "CancelId:0x%" PRIx32 " RequestId:0x%x endpoint 0x%x!!", RequestId, request->RequestId,
		         request->endpoint);
...
static IUDEVICE* udev_init(UDEVICE* pdev, UINT16 bus_number, UINT16 dev_number)
{
	...
	WLog_DBG(TAG,
	         ...
	         pdev->devDescriptor->idVendor, pdev->devDescriptor->idProduct, interface_temp.bInterfaceClass);
