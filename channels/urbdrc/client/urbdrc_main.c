...
#endif
	urbdrc->udevman->unregister_udevice(urbdrc->udevman, pdev->get_bus_number(pdev), pdev->get_dev_number(pdev));
}
...
static int func_container_id_generate(IUDEVICE* pdev, char* strContainerId)
{
	...
	sprintf_s(strContainerId, DEVICE_CONTAINER_STR_SIZE,
	          ...
	          "-%02" PRIx8 "%02" PRIx8 "-%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "}",
	          containerId[0], containerId[1], containerId[2], containerId[3], containerId[4], containerId[5],
	          containerId[6], containerId[7], containerId[8], containerId[9], containerId[10], containerId[11],
...
static int func_instance_id_generate(IUDEVICE* pdev, char* strInstanceId)
{
	...
	sprintf_s(strInstanceId, DEVICE_INSTANCE_STR_SIZE,
	          "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "-%02" PRIx8 "%02" PRIx8 "-%02" PRIx8 "%02" PRIx8
	          "-%02" PRIx8 "%02" PRIx8 "-%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "%02" PRIx8 "",
	          instanceId[0], instanceId[1], instanceId[2], instanceId[3], instanceId[4], instanceId[5], instanceId[6],
	          instanceId[7], instanceId[8], instanceId[9], instanceId[10], instanceId[11], instanceId[12],
	          instanceId[13], instanceId[14], instanceId[15]);
	return 0;
}
...
static void func_lock_isoch_mutex(TRANSFER_DATA* transfer_data)
{
	...
	if (transfer_data->cbSize >= 8)
	{
		...
		if ((FunctionId == TRANSFER_IN_REQUEST || FunctionId == TRANSFER_OUT_REQUEST) && transfer_data->cbSize >= 16)
		{
 ...
 */
static UINT urbdrc_process_capability_request(URBDRC_CHANNEL_CALLBACK* callback, char* data, UINT32 data_sizem,
                                              UINT32 MessageId)
{
	UINT32 InterfaceId;
	UINT32 Version;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT urbdrc_process_channel_create(URBDRC_CHANNEL_CALLBACK* callback, char* data, UINT32 data_sizem,
                                          UINT32 MessageId)
{
	UINT32 InterfaceId;
	UINT32 out_size;
...
static UINT urdbrc_send_usb_device_add(URBDRC_CHANNEL_CALLBACK* callback, IUDEVICE* pdev)
{
	...
	cchCompatIds = strlen(CompatibilityIds[0]) + 1 + strlen(CompatibilityIds[1]) + 1 + strlen(CompatibilityIds[2]) + 2;
	...
	out_offset = 24;
	size = 24;
	size += (strlen(strInstanceId) + 1) * 2 + (strlen(HardwareIds[0]) + 1) * 2 + 4 + (strlen(HardwareIds[1]) + 1) * 2 +
	  2 + 4 + (cchCompatIds) *2 + (strlen(strContainerId) + 1) * 2 + 4 + 28;
...
static void* urbdrc_search_usb_device(void* arg)
{
	...
	while (WaitForMultipleObjects(2, listobj, FALSE, INFINITE) != WAIT_OBJECT_0)
	{
		...
		errno = 0;
		WLog_DBG(TAG, "=======  SEARCH  ======= ");
		/* !system=USB subsystem=DEVICE type=ATTACH ugen=ugen3.3 cdev=ugen3.3 vendor=0x046d product=0x082d devclass=0xef
		 * devsubclass=0x02 sernum="6E7D726F" release=0x0011 mode=host port=4 parent=ugen3.1 */
		/* !system=USB subsystem=DEVICE type=DETACH ugen=ugen3.3 cdev=ugen3.3 vendor=0x046d product=0x082d devclass=0xef
		 * devsubclass=0x02 sernum="6E7D726F" release=0x0011 mode=host port=4 parent=ugen3.1 */
		...
		switch (action)
		{
			case 0: /* ATTACH */
				...
				while (dvc_channel && searchman->has_next(searchman))
				{
					...
					{
						WLog_VRB(TAG, "Searchman Found Device: %04" PRIx16 ":%04" PRIx16 "", sdev->idVendor,
						         sdev->idProduct);
						found = 1;
						break;
					}
				...
				if (found)
				{
					success = udevman->register_udevice(udevman, busnum, devnum, searchman->UsbDevice, 0, 0,
			...
			if (dev)
			{
				...
				if (strcmp(action, "add") == 0)
				{
					...
					while (dvc_channel && searchman->has_next(searchman))
					{
						...
						if (sdev->idVendor == idVendor && sdev->idProduct == idProduct)
						{
							WLog_VRB(TAG, "Searchman Find Device: %04" PRIx16 ":%04" PRIx16 "", sdev->idVendor,
							         sdev->idProduct);
							found = 1;
							break;
						}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT urbdrc_on_new_channel_connection(IWTSListenerCallback* pListenerCallback, IWTSVirtualChannel* pChannel,
                                             BYTE* pData, BOOL* pbAccept, IWTSVirtualChannelCallback** ppCallback)
{
...
static UINT urbdrc_plugin_initialize(IWTSPlugin* pPlugin, IWTSVirtualChannelManager* pChannelMgr)
{
	...
	return pChannelMgr->CreateListener(pChannelMgr, "URBDRC", 0, (IWTSListenerCallback*) urbdrc->listener_callback,
	                                   NULL);
}
...
static UINT urbdrc_process_addin_args(URBDRC_PLUGIN* urbdrc, ADDIN_ARGV* args)
{
	...
	status = CommandLineParseArgumentsA(args->argc, args->argv, urbdrc_args, flags, urbdrc, NULL, NULL);
