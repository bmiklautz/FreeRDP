...
static PresentationContext* PresentationContext_new(VideoClientContext* video, BYTE PresentationId, UINT32 x, UINT32 y,
...
static BOOL video_onMappedGeometryUpdate(MAPPED_GEOMETRY* geometry)
{
	...
	WLog_DBG(TAG, "geometry updated topGeom=(%d,%d-%dx%d) geom=(%d,%d-%dx%d) rects=(%d,%d-%dx%d)",
	         geometry->topLevelLeft, geometry->topLevelTop, geometry->topLevelRight - geometry->topLevelLeft,
	         geometry->topLevelBottom - geometry->topLevelTop,

	         geometry->left, geometry->top, geometry->right - geometry->left, geometry->bottom - geometry->top,
...
static UINT video_PresentationRequest(VideoClientContext* video, TSMM_PRESENTATION_REQUEST* req)
{
	...
	if (req->Command == TSMM_START_PRESENTATION)
	{
		...
		presentation = PresentationContext_new(video, req->PresentationId, geom->topLevelLeft + geom->left,
...
static UINT video_read_tsmm_presentation_req(VideoClientContext* context, wStream* s)
{
	...
	WLog_DBG(TAG,
	         ...
	         req.PresentationId, req.Version, video_command_name(req.Command), req.SourceWidth, req.SourceHeight,
	         req.ScaledWidth, req.ScaledHeight, req.hnsTimestampOffset, req.GeometryMappingId);
...
treat_feedback:
	if (priv->nextFeedbackTime < now)
	{
		...
		WLog_DBG(TAG, "currentRate=%d published=%d dropped=%d", priv->lastSentRate, priv->publishedFrames,
		         priv->droppedFrames);
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT video_control_on_new_channel_connection(IWTSListenerCallback* listenerCallback, IWTSVirtualChannel* channel,
...
static UINT video_data_on_new_channel_connection(IWTSListenerCallback* pListenerCallback, IWTSVirtualChannel* pChannel,
                                                 BYTE* Data, BOOL* pbAccept, IWTSVirtualChannelCallback** ppCallback)
{
	VIDEO_CHANNEL_CALLBACK* callback;
...
static UINT video_plugin_initialize(IWTSPlugin* plugin, IWTSVirtualChannelManager* channelMgr)
{
	...
	status = channelMgr->CreateListener(channelMgr, VIDEO_CONTROL_DVC_CHANNEL_NAME, 0, (IWTSListenerCallback*) callback,
	                                    &(video->controlListener));
	...
	status = channelMgr->CreateListener(channelMgr, VIDEO_DATA_DVC_CHANNEL_NAME, 0, (IWTSListenerCallback*) callback,
	                                    &(video->dataListener));
