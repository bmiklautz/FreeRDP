...
static void xf_draw_screen_scaled(xfContext* xfc, int x, int y, int w, int h)
{
	...
	XRenderComposite(xfc->display, PictOpSrc, primaryPicture, 0, windowPicture, x, y, 0, 0, xfc->offset_x + x,
	                 xfc->offset_y + y, w, h);
...
static BOOL xf_desktop_resize(rdpContext* context)
{
	...
	if (xfc->primary)
	{
		...
		if (!(xfc->primary = XCreatePixmap(xfc->display, xfc->drawable, settings->DesktopWidth, settings->DesktopHeight,
		                                   xfc->depth)))
			return FALSE;
...
static BOOL xf_sw_desktop_resize(rdpContext* context)
{
	...
	if (!(xfc->image = XCreateImage(xfc->display, xfc->visual, xfc->depth, ZPixmap, 0, (char*) gdi->primary_buffer,
...
BOOL xf_create_window(xfContext* xfc)
{
	...
	if (!xfc->remote_app)
	{
		...
		{
			size_t size = 1 + sizeof("FreeRDP: ") + strlen(settings->ServerHostname) + sizeof(":00000");
			windowTitle = malloc(size);

			if (!windowTitle)
				return FALSE;

			sprintf_s(windowTitle, size, "FreeRDP: %s:%i", settings->ServerHostname, settings->ServerPort);
		}
	...
	if (!xfc->primary)
		xfc->primary =
		  XCreatePixmap(xfc->display, xfc->drawable, settings->DesktopWidth, settings->DesktopHeight, xfc->depth);
	...
	XFillRectangle(xfc->display, xfc->primary, xfc->gc, 0, 0, settings->DesktopWidth, settings->DesktopHeight);
	XFlush(xfc->display);

	if (!xfc->image)
	{
		rdpGdi* gdi = xfc->context.gdi;
		xfc->image = XCreateImage(xfc->display, xfc->visual, xfc->depth, ZPixmap, 0, (char*) gdi->primary_buffer,
		                          settings->DesktopWidth, settings->DesktopHeight, xfc->scanline_pad, gdi->stride);
		xfc->image->byte_order = LSBFirst;
		xfc->image->bitmap_bit_order = LSBFirst;
	}
...
void xf_encomsp_uninit(xfContext* xfc, EncomspClientContext* encomsp) { xfc->encomsp = NULL; }
...
static void xf_check_extensions(xfContext* context)
{
	...
	if (XkbLibraryVersion(&xkb_major, &xkb_minor) &&
	    XkbQueryExtension(context->display, &xkb_opcode, &xkb_event, &xkb_error, &xkb_major, &xkb_minor))
	{
...
static const int xf_button_flags[NUM_BUTTONS_MAPPED] = { PTR_FLAGS_BUTTON1, PTR_FLAGS_BUTTON3, PTR_FLAGS_BUTTON2 };
...
static void xf_button_map_init(xfContext* xfc)
{
	...
	unsigned char x11_map[NUM_BUTTONS_MAPPED] = { Button1, Button2, Button3 };
	...
	for (physical = 0; physical < NUM_BUTTONS_MAPPED; ++physical)
	{
		...
		{
			WLog_ERR(TAG, "Mouse physical button %d is mapped to logical button %d", physical, logical);
		}
	}
}
...
static BOOL xf_pre_connect(freerdp* instance)
{
	...
	PubSub_SubscribeChannelDisconnected(instance->context->pubSub, xf_OnChannelDisconnectedEventHandler);
	 ...
	 */
	if (settings->Fullscreen && settings->SmartSizing && settings->SmartSizingWidth && settings->SmartSizingHeight)
	{
...
static void xf_post_disconnect(freerdp* instance)
{
	...
	PubSub_UnsubscribeChannelConnected(instance->context->pubSub, xf_OnChannelConnectedEventHandler);
	PubSub_UnsubscribeChannelDisconnected(instance->context->pubSub, xf_OnChannelDisconnectedEventHandler);
	gdi_free(instance);
...
static DWORD WINAPI xf_client_thread(LPVOID param)
{
	...
	if (!exit_code)
	{
		    ...
		    freerdp_get_disconnect_ultimatum(context) == Disconnect_Ultimatum_user_requested)
		{
			...
			WLog_INFO(TAG,
			          "Error info says user did not initiate but disconnect ultimatum says they did; treat this as a "
			          "user logoff");
...
static BOOL xfreerdp_client_new(freerdp* instance, rdpContext* context)
{
	...
	if ((xfc->_NET_SUPPORTED != None) && (xfc->_NET_SUPPORTING_WM_CHECK != None))
	{
		...
		int status = XGetWindowProperty(xfc->display, RootWindowOfScreen(xfc->screen), xfc->_NET_SUPPORTED, 0, 1024,
	...
	xfc->_NET_WM_WINDOW_TYPE_POPUP_MENU = XInternAtom(xfc->display, "_NET_WM_WINDOW_TYPE_POPUP_MENU", False);
