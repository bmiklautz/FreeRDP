...
void xf_keyboard_clear(xfContext* xfc) { ZeroMemory(xfc->KeyboardState, 256 * sizeof(BOOL)); }
...
int xf_keyboard_read_keyboard_state(xfContext* xfc)
{
	...
	if (!xfc->remote_app)
	{
		XQueryPointer(xfc->display, xfc->window->handle, &wdummy, &wdummy, &dummy, &dummy, &dummy, &dummy, &state);
	}
	else
	{
		XQueryPointer(xfc->display, DefaultRootWindow(xfc->display), &wdummy, &wdummy, &dummy, &dummy, &dummy, &dummy,
		              &state);
	}
...
static void xk_keyboard_update_modifier_keys(xfContext* xfc)
{
	...
	size_t i;
	KeyCode keycode;
	int keysyms[] = { XK_Shift_L, XK_Shift_R, XK_Alt_L, XK_Alt_R, XK_Control_L, XK_Control_R, XK_Super_L, XK_Super_R };
	state = xf_keyboard_read_keyboard_state(xfc);
...
static int xf_keyboard_execute_action_script(xfContext* xfc, XF_MODIFIER_KEYS* mod, KeySym keysym)
{
	...
	if ((keysym == XK_Shift_L) || (keysym == XK_Shift_R) || (keysym == XK_Alt_L) || (keysym == XK_Alt_R) ||
	...
	sprintf_s(command, sizeof(command), "%s key %s", xfc->context.settings->ActionScript, combination);
	keyScript = popen(command, "r");
