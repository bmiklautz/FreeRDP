...
static VideoSurface* xfVideoCreateSurface(VideoClientContext* video, BYTE* data, UINT32 x, UINT32 y, UINT32 width,
                                          UINT32 height)
{
	...
	ret->base.h = height;
	ret->image =
	  XCreateImage(xfc->display, xfc->visual, xfc->depth, ZPixmap, 0, (char*) data, width, height, 8, width * 4);
...
static BOOL xfVideoShowSurface(VideoClientContext* video, VideoSurface* surface)
{
	...
	{
		XPutImage(xfc->display, xfc->primary, xfc->gc, xfSurface->image, 0, 0, surface->x, surface->y, surface->w,
		          surface->h);
...
#endif
	{
		XPutImage(xfc->display, xfc->drawable, xfc->gc, xfSurface->image, 0, 0, surface->x, surface->y, surface->w,
		          surface->h);
	}
