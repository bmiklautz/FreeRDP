...
static void xf_SetWindowTitleText(xfContext* xfc, Window window, const char* name)
{
	...
	XChangeProperty(xfc->display, window, wm_Name, utf8Str, 8, PropModeReplace, (const unsigned char*) name, (int) i);
}
...
void xf_SendClientEvent(xfContext* xfc, Window window, Atom atom, unsigned int numArgs, ...)
{
	...
	XSendEvent(xfc->display, RootWindowOfScreen(xfc->screen), False, SubstructureRedirectMask | SubstructureNotifyMask,
	           &xevent);
...
void xf_SetWindowFullscreen(xfContext* xfc, xfWindow* window, BOOL fullscreen)
{
	...
	if (xfc->_NET_WM_FULLSCREEN_MONITORS != None || settings->MonitorCount == 1)
	{
		...
		xf_SendClientEvent(xfc, window->handle, xfc->_NET_WM_STATE, 4,
		                   fullscreen ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE, xfc->_NET_WM_STATE_FULLSCREEN, 0, 0);
		...
		{
			xf_SendClientEvent(xfc, window->handle, xfc->_NET_WM_FULLSCREEN_MONITORS, 5, xfc->fullscreenMonitors.top,
			                   xfc->fullscreenMonitors.bottom, xfc->fullscreenMonitors.left,
			                   xfc->fullscreenMonitors.right, 1);
		}
	}
	else
	{
...
BOOL xf_GetWindowProperty(xfContext* xfc, Window window, Atom property, int length, unsigned long* nitems,
                          unsigned long* bytes, BYTE** prop)
{
	...
	status = XGetWindowProperty(xfc->display, window, property, 0, length, False, AnyPropertyType, &actual_type,
	                            &actual_format, nitems, bytes, prop);
...
BOOL xf_GetCurrentDesktop(xfContext* xfc)
{
	...
	unsigned char* prop;
	status =
	  xf_GetWindowProperty(xfc, DefaultRootWindow(xfc->display), xfc->_NET_CURRENT_DESKTOP, 1, &nitems, &bytes, &prop);
...
BOOL xf_GetWorkArea(xfContext* xfc)
{
	...
	status =
	  xf_GetWindowProperty(xfc, DefaultRootWindow(xfc->display), xfc->_NET_WORKAREA, 32 * 4, &nitems, &bytes, &prop);
...
void xf_SetWindowDecorations(xfContext* xfc, Window window, BOOL show)
{
	...
	hints.inputMode = 0;
	hints.status = 0;
	XChangeProperty(xfc->display, window, xfc->_MOTIF_WM_HINTS, xfc->_MOTIF_WM_HINTS, 32, PropModeReplace,
	                (BYTE*) &hints, PROP_MOTIF_WM_HINTS_ELEMENTS);
}
...
void xf_SetWindowUnlisted(xfContext* xfc, Window window)
{
	...
	XChangeProperty(xfc->display, window, xfc->_NET_WM_STATE, XA_ATOM, 32, PropModeReplace, (BYTE*) &window_state, 2);
}
...
static void xf_SetWindowPID(xfContext* xfc, Window window, pid_t pid)
{
	...
	XChangeProperty(xfc->display, window, am_wm_pid, XA_CARDINAL, 32, PropModeReplace, (BYTE*) &pid, 1);
}
...
xfWindow* xf_CreateDesktopWindow(xfContext* xfc, char* name, int width, int height)
{
	...
	window->handle = XCreateWindow(xfc->display, RootWindowOfScreen(xfc->screen), xfc->workArea.x, xfc->workArea.y,
	                               xfc->workArea.width, xfc->workArea.height, 0, xfc->depth, InputOutput, xfc->visual,
	                               CWBackPixel | CWBackingStore | CWOverrideRedirect | CWColormap | CWBorderPixel |
	...
	input_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | VisibilityChangeMask |
	  FocusChangeMask | StructureNotifyMask | PointerMotionMask | ExposureMask | PropertyChangeMask;

	if (xfc->grab_keyboard)
		input_mask |= EnterWindowMask | LeaveWindowMask;

	XChangeProperty(xfc->display, window->handle, xfc->_NET_WM_ICON, XA_CARDINAL, 32, PropModeReplace,
	                (BYTE*) xf_icon_prop, ARRAYSIZE(xf_icon_prop));
...
void xf_SetWindowStyle(xfContext* xfc, xfAppWindow* appWindow, UINT32 style, UINT32 ex_style)
{
	...
	XChangeProperty(xfc->display, appWindow->handle, xfc->_NET_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace,
	                (BYTE*) &window_type, 1);
}
...
int xf_AppWindowInit(xfContext* xfc, xfAppWindow* appWindow)
{
	...
	appWindow->handle = XCreateWindow(xfc->display, RootWindowOfScreen(xfc->screen), appWindow->x, appWindow->y,
	                                  appWindow->width, appWindow->height, 0, xfc->depth, InputOutput, xfc->visual, 0,
	                                  &xfc->attribs);
	...
	input_mask = KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | EnterWindowMask |
	  LeaveWindowMask | PointerMotionMask | Button1MotionMask | Button2MotionMask | Button3MotionMask |
	  Button4MotionMask | Button5MotionMask | ButtonMotionMask | KeymapStateMask | ExposureMask | VisibilityChangeMask |
	  StructureNotifyMask | SubstructureNotifyMask | SubstructureRedirectMask | FocusChangeMask | PropertyChangeMask |
...
void xf_SetWindowMinMaxInfo(xfContext* xfc, xfAppWindow* appWindow, int maxWidth, int maxHeight, int maxPosX,
                            int maxPosY, int minTrackWidth, int minTrackHeight, int maxTrackWidth, int maxTrackHeight)
{
	XSizeHints* size_hints;
...
void xf_ShowWindow(xfContext* xfc, xfAppWindow* appWindow, BYTE state)
{
	switch (state)
	{
		...
		case WINDOW_SHOW_MAXIMIZED:
			...
			/*
			 * This is a workaround for the case where the window is maximized locally before the rail server is told to
			 * maximize the window, this appears to be a race condition where the local window with incomplete data and
			 * once the window is actually maximized on the server - an update of the new areas may not happen. So, we
			 * simply to do a full update of the entire window once the rail server notifies us that the window is now
			 * maximized.
			 */
...
void xf_SetWindowVisibilityRects(xfContext* xfc, xfAppWindow* appWindow, UINT32 rectsOffsetX, UINT32 rectsOffsetY,
                                 RECTANGLE_16* rects, int nrects)
{
	...
	XShapeCombineRectangles(xfc->display, appWindow->handle, ShapeBounding, rectsOffsetX, rectsOffsetY, xrects, nrects,
	                        ShapeSet, 0);
	free(xrects);
#endif
}
...
void xf_UpdateWindowArea(xfContext* xfc, xfAppWindow* appWindow, int x, int y, int width, int height)
{
	...
	{
		XPutImage(xfc->display, xfc->primary, appWindow->gc, xfc->image, ax, ay, ax, ay, width, height);
	}

	XCopyArea(xfc->display, xfc->primary, appWindow->handle, appWindow->gc, ax, ay, width, height, x, y);
	XFlush(xfc->display);
