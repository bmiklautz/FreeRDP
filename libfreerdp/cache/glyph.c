...
static UINT32 update_glyph_offset(const BYTE* data, UINT32 index, INT32* x, INT32* y, UINT32 ulCharInc, UINT32 flAccel)
{
...
static BOOL update_process_glyph(rdpContext* context, const BYTE* data, UINT32 cacheIndex, INT32* x, INT32* y,
                                 UINT32 cacheId, UINT32 flAccel, BOOL fOpRedundant, const RDP_RECT* bound)
{
	INT32 sx = 0, sy = 0;
	INT32 dx, dy;
	rdpGlyph* glyph;
	...
	if (!context || !data || !x || !y || !context->graphics || !context->cache || !context->cache->glyph)
		return FALSE;
...
static BOOL update_process_glyph_fragments(rdpContext* context, const BYTE* data, UINT32 length, UINT32 cacheId,
                                           UINT32 ulCharInc, UINT32 flAccel, UINT32 bgcolor, UINT32 fgcolor, INT32 x,
                                           INT32 y, INT32 bkX, INT32 bkY, INT32 bkWidth, INT32 bkHeight, INT32 opX,
                                           INT32 opY, INT32 opWidth, INT32 opHeight, BOOL fOpRedundant)
{
	UINT32 n;
	UINT32 id;
	UINT32 size;
		...
		switch (op)
		{
			case GLYPH_FRAGMENT_USE:
				...
				for (n = 0; n < size;)
				{
					...
					if (!update_process_glyph(context, fragments, fop, &x, &y, cacheId, flAccel, fOpRedundant, &bound))
						return FALSE;
				}
			...
			default:
				...
				if (!update_process_glyph(context, data, op, &x, &y, cacheId, flAccel, fOpRedundant, &bound))
					return FALSE;
...
static BOOL update_gdi_glyph_index(rdpContext* context, GLYPH_INDEX_ORDER* glyphIndex)
{
	...
	return update_process_glyph_fragments(
	  context, glyphIndex->data, glyphIndex->cbData, glyphIndex->cacheId, glyphIndex->ulCharInc, glyphIndex->flAccel,
	  glyphIndex->backColor, glyphIndex->foreColor, glyphIndex->x, glyphIndex->y, glyphIndex->bkLeft, glyphIndex->bkTop,
	  bkWidth, bkHeight, glyphIndex->opLeft, glyphIndex->opTop, opWidth, opHeight, glyphIndex->fOpRedundant);
}
...
static BOOL update_gdi_fast_index(rdpContext* context, const FAST_INDEX_ORDER* fastIndex)
{
	...
	return update_process_glyph_fragments(context, fastIndex->data, fastIndex->cbData, fastIndex->cacheId,
	                                      fastIndex->ulCharInc, fastIndex->flAccel, fastIndex->backColor,
	                                      fastIndex->foreColor, x, y, fastIndex->bkLeft, fastIndex->bkTop, bkWidth,
	                                      bkHeight, opLeft, opTop, opWidth, opHeight, FALSE);
}
...
static BOOL update_gdi_fast_glyph(rdpContext* context, const FAST_GLYPH_ORDER* fastGlyph)
{
	...
	if ((fastGlyph->cbData > 1) && (fastGlyph->glyphData.aj))
	{
		...
		glyph =
		  Glyph_Alloc(context, glyphData->x, glyphData->y, glyphData->cx, glyphData->cy, glyphData->cb, glyphData->aj);
	...
	return update_process_glyph_fragments(context, text_data, 1, fastGlyph->cacheId, fastGlyph->ulCharInc,
	                                      fastGlyph->flAccel, fastGlyph->backColor, fastGlyph->foreColor, x, y,
	                                      fastGlyph->bkLeft, fastGlyph->bkTop, bkWidth, bkHeight, opLeft, opTop,
...
static BOOL update_gdi_cache_glyph(rdpContext* context, const CACHE_GLYPH_ORDER* cacheGlyph)
{
	...
	for (i = 0; i < cacheGlyph->cGlyphs; i++)
	{
		...
		if (!(glyph = Glyph_Alloc(context, glyph_data->x, glyph_data->y, glyph_data->cx, glyph_data->cy, glyph_data->cb,
		                          glyph_data->aj)))
			return FALSE;
...
{
	rdpGlyph* glyph;
	WLog_Print(glyphCache->log, WLOG_DEBUG, "GlyphCacheGet: id: %" PRIu32 " index: %" PRIu32 "", id, index);
...
BOOL glyph_cache_put(rdpGlyphCache* glyphCache, UINT32 id, UINT32 index, rdpGlyph* glyph)
{
	...
	WLog_Print(glyphCache->log, WLOG_DEBUG, "GlyphCachePut: id: %" PRIu32 " index: %" PRIu32 "", id, index);
...
const void* glyph_cache_fragment_get(rdpGlyphCache* glyphCache, UINT32 index, UINT32* size)
{
	...
	*size = (BYTE) glyphCache->fragCache.entries[index].size;
	WLog_Print(glyphCache->log, WLOG_DEBUG, "GlyphCacheFragmentGet: index: %" PRIu32 " size: %" PRIu32 "", index,
	           *size);
...
BOOL glyph_cache_fragment_put(rdpGlyphCache* glyphCache, UINT32 index, UINT32 size, const void* fragment)
{
	void* prevFragment;
	void* copy;
	...
	WLog_Print(glyphCache->log, WLOG_DEBUG, "GlyphCacheFragmentPut: index: %" PRIu32 " size: %" PRIu32 "", index, size);
