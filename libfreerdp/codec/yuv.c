...
static void CALLBACK yuv_process_work_callback(PTP_CALLBACK_INSTANCE instance, void* context, PTP_WORK work)
{
	...
	if (prims->YUV420ToRGB_8u_P3AC4R(param->pYUVData, param->iStride, param->dest, param->nDstStep, param->DstFormat,
...
BOOL yuv_context_decode(YUV_CONTEXT* context, const BYTE* pYUVData[3], UINT32 iStride[3], DWORD DstFormat, BYTE* dest,
                        UINT32 nDstStep)
{
	UINT32 y, nobjects, i;
	...
	if (!context->useThreads)
	{
		...
		return prims->YUV420ToRGB_8u_P3AC4R(pYUVData, iStride, dest, nDstStep, DstFormat, &roi) == PRIMITIVES_SUCCESS;
	}
	...
	for (i = 0, y = 0; i < nobjects; i++, y += context->heightStep, waitCount++)
	{
		...
		work_objects[i] = CreateThreadpoolWork(yuv_process_work_callback, (void*) &params[i], &context->ThreadPoolEnv);
		if (!work_objects[i])
		{
			ret = FALSE;
			break;
