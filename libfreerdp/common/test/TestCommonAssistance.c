...
static BOOL test_msrsc_incident_file_type1(wLog* log)
{
	...
	status = freerdp_assistance_parse_file_buffer(
	  file, TEST_MSRC_INCIDENT_FILE_TYPE1, sizeof(TEST_MSRC_INCIDENT_FILE_TYPE1), TEST_MSRC_INCIDENT_PASSWORD_TYPE1);
	...
	WLog_Print(log, WLOG_INFO, "freerdp_assistance_decrypt: %d %s [%" PRIdz "]", status, pass, EncryptedPassStubLength);
...
static BOOL test_msrsc_incident_file_type2(wLog* log)
{
	...
	status = freerdp_assistance_parse_file_buffer(
	  file, TEST_MSRC_INCIDENT_FILE_TYPE2, sizeof(TEST_MSRC_INCIDENT_FILE_TYPE2), TEST_MSRC_INCIDENT_PASSWORD_TYPE2);
	...
	status = freerdp_assistance_get_encrypted_pass_stub(file, &EncryptedPassStub, &EncryptedPassStubLength);
	pass = freerdp_assistance_bin_to_hex_string(EncryptedPassStub, EncryptedPassStubLength);
