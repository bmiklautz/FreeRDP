...
static BOOL fastpath_write_update_pdu_header(wStream* s, FASTPATH_UPDATE_PDU_HEADER* fpUpdatePduHeader, rdpRdp* rdp)
{
...
static int fastpath_recv_update(rdpFastPath* fastpath, BYTE updateCode, wStream* s)
{
	...
	DEBUG_RDP("recv Fast-Path %s Update (0x%02" PRIX8 "), length:%" PRIuz "", fastpath_update_to_string(updateCode),
	          updateCode, Stream_GetRemainingLength(s));
#endif
	...
	if (!rc)
	{
		WLog_ERR(TAG, "Fastpath update %s [%" PRIx8 "] failed, status %d", fastpath_update_to_string(updateCode),
		         updateCode, status);
		return -1;
	}
...
static int fastpath_recv_update_data(rdpFastPath* fastpath, wStream* s)
{
	...
	else
	{
		...
		{
			WLog_ERR(TAG, "Total size (%" PRIuz ") exceeds MultifragMaxRequestSize (%" PRIu32 ")", totalSize,
			         transport->settings->MultifragMaxRequestSize);
			goto out_fail;
		}
...
static BOOL fastpath_recv_input_event_unicode(rdpFastPath* fastpath, wStream* s, BYTE eventFlags)
{
	...
	return IFCALLRESULT(FALSE, fastpath->rdp->input->UnicodeKeyboardEvent, fastpath->rdp->input, flags, unicodeCode);
}
