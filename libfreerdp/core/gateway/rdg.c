...
static const t_err_mapping extended_auth[] = { { HTTP_EXTENDED_AUTH_NONE, "HTTP_EXTENDED_AUTH_NONE" },
...
static BOOL rdg_process_handshake_response(rdpRdg* rdg, wStream* s)
{
	...
	WLog_DBG(TAG, "errorCode=%s, verMajor=%" PRId8 ", verMinor=%" PRId8 ", serverVersion=%" PRId16 ", extendedAuth=%s",
...
static BOOL rdg_establish_data_connection(rdpRdg* rdg, rdpTls* tls, const char* method, const char* peerAddress,
...
rdpRdg* rdg_new(rdpContext* context)
{
	...
	if (rdg)
	{
		...
		if (!http_context_set_uri(rdg->http, "/remoteDesktopGateway/") || !http_context_set_accept(rdg->http, "*/*") ||
		    !http_context_set_cache_control(rdg->http, "no-cache") || !http_context_set_pragma(rdg->http, "no-cache") ||
