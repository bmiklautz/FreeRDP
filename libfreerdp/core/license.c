...
static void license_print_product_info(const LICENSE_PRODUCT_INFO* productInfo)
{
	...
	char* ProductId = NULL;
	ConvertFromUnicode(CP_UTF8, 0, (WCHAR*) productInfo->pbCompanyName, productInfo->cbCompanyName / 2, &CompanyName, 0,
	                   NULL, NULL);
	ConvertFromUnicode(CP_UTF8, 0, (WCHAR*) productInfo->pbProductId, productInfo->cbProductId / 2, &ProductId, 0, NULL,
	                   NULL);
...
static BOOL license_generate_keys(rdpLicense* license)
{
	...
	if (
	  /* MasterSecret */
	  !security_master_secret(license->PremasterSecret, license->ClientRandom, license->ServerRandom,
	                          license->MasterSecret) ||
	  /* SessionKeyBlob */
	  !security_session_key_blob(license->MasterSecret, license->ClientRandom, license->ServerRandom,
	                             license->SessionKeyBlob))
	{
		return FALSE;
	}
	security_mac_salt_key(license->SessionKeyBlob, license->ClientRandom, license->ServerRandom,
	                      license->MacSaltKey); /* MacSaltKey */
	ret = security_licensing_encryption_key(license->SessionKeyBlob, license->ClientRandom, license->ServerRandom,
...
BOOL license_generate_hwid(rdpLicense* license)
{
	...
	/* Allow FIPS override for use of MD5 here, really this does not have to be MD5 as we are just taking a MD5 hash of
	 * the 6 bytes of 0's(macAddress) */
	/* and filling in the Data1-Data4 fields of the CLIENT_HARDWARE_ID structure(from MS-RDPELE section 2.2.2.3.1). This
	 * is for RDP licensing packets */
	/* which will already be encrypted under FIPS, so the use of MD5 here is not for sensitive data protection. */
	return winpr_Digest_Allow_FIPS(WINPR_MD_MD5, hashTarget, targetLen, &license->HardwareId[HWID_PLATFORM_ID_LENGTH],
...
BOOL license_encrypt_premaster_secret(rdpLicense* license)
{
	...
	license->EncryptedPremasterSecret->length =
	  crypto_rsa_public_encrypt(license->PremasterSecret, PREMASTER_SECRET_LENGTH, license->ModulusLength,
 ...
 * @return if the operation completed successfully
 */
static BOOL license_decrypt_and_check_MAC(rdpLicense* license, const BYTE* input, size_t len, LICENSE_BLOB* target,
...
SCOPE_LIST* license_new_scope_list() { return (SCOPE_LIST*) calloc(1, sizeof(SCOPE_LIST)); }
...
BOOL license_read_platform_challenge_packet(rdpLicense* license, wStream* s)
{
	...
	if (!license_decrypt_and_check_MAC(license, license->EncryptedPlatformChallenge->data,
	                                   license->EncryptedPlatformChallenge->length, license->PlatformChallenge,
	                                   macData))
		return FALSE;
...
BOOL license_answer_license_request(rdpLicense* license)
{
	...
	if (license_data)
	{
		...
		if (!license_encrypt_and_MAC(license, license->HardwareId, HWID_LENGTH, license->EncryptedHardwareId,
		                             signature))
		{
			free(license_data);
			return FALSE;
...
BOOL license_write_platform_challenge_response_packet(rdpLicense* license, wStream* s, const BYTE* macData)
{
	if (!license_write_binary_blob(
	      s, license->EncryptedPlatformChallengeResponse) || /* EncryptedPlatformChallengeResponse */
...
BOOL license_send_platform_challenge_response_packet(rdpLicense* license)
{
	...
	status = license_rc4_with_licenseKey(license, Stream_Buffer(challengeRespData), Stream_Length(challengeRespData),
