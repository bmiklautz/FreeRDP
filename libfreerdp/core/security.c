...
static const BYTE fips_oddparity_table[256] = {
	
};

static BOOL security_salted_hash(const BYTE* salt, const BYTE* input, int length, const BYTE* salt1, const BYTE* salt2,
                                 BYTE* output)
{
	...
	/* Allow FIPS override for use of MD5 here, this is used for creating hashes of the premaster_secret and
	 * master_secret */
...
BOOL security_master_secret(const BYTE* premaster_secret, const BYTE* client_random, const BYTE* server_random,
                            BYTE* output)
{
...
static BOOL security_master_hash(const char* input, int length, const BYTE* master_secret, const BYTE* client_random,
...
BOOL security_session_key_blob(const BYTE* master_secret, const BYTE* client_random, const BYTE* server_random,
                               BYTE* output)
{
...
void security_mac_salt_key(const BYTE* session_key_blob, const BYTE* client_random, const BYTE* server_random,
                           BYTE* output)
{
                                       ...
                                       const BYTE* server_random, BYTE* output)
{
	 ...
	 * Allow FIPS use of MD5 here, this is just used for creating the licensing encryption key as described in
	 * MS-RDPELE. This is for RDP licensing packets which will already be encrypted under FIPS, so the use of MD5 here
	 * is not for sensitive data protection. */
...
BOOL security_establish_keys(const BYTE* client_random, rdpRdp* rdp)
{
	...
	if (settings->EncryptionMethods == ENCRYPTION_METHOD_FIPS)
	{
		...
		if (!winpr_Digest_Init(sha1, WINPR_MD_SHA1) || !winpr_Digest_Update(sha1, client_random + 16, 16) ||
		...
		if (!winpr_Digest_Init(sha1, WINPR_MD_SHA1) || !winpr_Digest_Update(sha1, client_random, 16) ||
	...
	else
	{
		...
		status =
		  security_md5_16_32_32_Allow_FIPS(&session_key_blob[16], client_random, server_random, rdp->decrypt_key);
		status &=
		  security_md5_16_32_32_Allow_FIPS(&session_key_blob[32], client_random, server_random, rdp->encrypt_key);
	}
