...
static BOOL _update_read_pointer_color(wStream* s, POINTER_COLOR_UPDATE* pointer_color, BYTE xorBpp)
{
	...
	if (pointer_color->lengthAndMask > 0)
	{
		...
		{
			WLog_ERR(TAG, "invalid lengthAndMask: %" PRIu32 " instead of %" PRIu32 "", pointer_color->lengthAndMask,
			         scanlineSize * pointer_color->height);
			goto fail;
		}
...
POINTER_NEW_UPDATE* update_read_pointer_new(rdpUpdate* update, wStream* s)
{
	...
	if (!_update_read_pointer_color(s, &pointer_new->colorPtrAttr, pointer_new->xorBpp)) /* colorPtrAttr */
		goto fail;
...
{
	if ((bounds1->left == bounds2->left) && (bounds1->top == bounds2->top) && (bounds1->right == bounds2->right) &&
...
static BOOL update_send_surface_frame_bits(rdpContext* context, const SURFACE_BITS_COMMAND* cmd, BOOL first, BOOL last,
                                           UINT32 frameId)
{
	wStream* s;
	rdpRdp* rdp = context->rdp;
...
static BOOL update_send_desktop_resize(rdpContext* context) { return rdp_server_reactivate(context->rdp); }
...
static BOOL update_send_cache_bitmap_v2(rdpContext* context, CACHE_BITMAP_V2_ORDER* cache_bitmap_v2)
{
	...
	update_check_flush(
	  context,
	  headerLength +
	    update_approximate_cache_bitmap_v2_order(cache_bitmap_v2, cache_bitmap_v2->compressed, &extraFlags));
	s = update->us;
...
int update_process_messages(rdpUpdate* update) { return update_message_queue_process_pending_messages(update); }
