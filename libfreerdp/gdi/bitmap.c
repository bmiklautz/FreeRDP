...
HGDI_BITMAP gdi_CreateBitmapEx(UINT32 nWidth, UINT32 nHeight, UINT32 format, UINT32 stride, BYTE* data,
                               void (*fkt_free)(void*))
{
...
static INLINE BOOL BitBlt_write(HGDI_DC hdcDest, HGDI_DC hdcSrc, INT32 nXDest, INT32 nYDest, INT32 nXSrc, INT32 nYSrc,
                                INT32 x, INT32 y, BOOL useSrc, BOOL usePat, UINT32 style, const char* rop,
...
static BOOL adjust_src_dst_coordinates(HGDI_DC hdcDest, INT32* pnXSrc, INT32* pnYSrc, INT32* pnXDst, INT32* pnYDst,
                                       INT32* pnWidth, INT32* pnHeight)
{
	HGDI_BITMAP hDstBmp;
...
static BOOL BitBlt_process(HGDI_DC hdcDest, INT32 nXDest, INT32 nYDest, INT32 nWidth, INT32 nHeight, HGDI_DC hdcSrc,
                           INT32 nXSrc, INT32 nYSrc, const char* rop, const gdiPalette* palette)
{
	...
	if ((nXDest > nXSrc) && (nYDest > nYSrc))
	{
		for (y = nHeight - 1; y >= 0; y--)
		{
			...
			{
				if (!BitBlt_write(hdcDest, hdcSrc, nXDest, nYDest, nXSrc, nYSrc, x, y, useSrc, usePat, style, rop,
				                  palette))
					return FALSE;
			}
		}
 ...
 */
BOOL gdi_BitBlt(HGDI_DC hdcDest, INT32 nXDest, INT32 nYDest, INT32 nWidth, INT32 nHeight, HGDI_DC hdcSrc, INT32 nXSrc,
                INT32 nYSrc, DWORD rop, const gdiPalette* palette)
{
	...
	switch (rop)
	{
		case GDI_SRCCOPY:
			...
			if (!freerdp_image_copy(hDstBmp->data, hDstBmp->format, hDstBmp->scanline, nXDest, nYDest, nWidth, nHeight,
			                        hSrcBmp->data, hSrcBmp->format, hSrcBmp->scanline, nXSrc, nYSrc, palette,
			                        FREERDP_FLIP_NONE))
				return FALSE;
		...
		default:
			if (!BitBlt_process(hdcDest, nXDest, nYDest, nWidth, nHeight, hdcSrc, nXSrc, nYSrc, gdi_rop_to_string(rop),
			                    palette))
				return FALSE;
