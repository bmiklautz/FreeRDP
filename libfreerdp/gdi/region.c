...
INLINE BOOL gdi_CopyOverlap(INT32 x, INT32 y, INT32 width, INT32 height, INT32 srcx, INT32 srcy)
{
	...
	return (dst.right >= src.left && dst.left <= src.right && dst.bottom >= src.top && dst.top <= src.bottom) ? TRUE
	                                                                                                          : FALSE;
}
...
{
	if ((hSrcRgn1->x == hSrcRgn2->x) && (hSrcRgn1->y == hSrcRgn2->y) && (hSrcRgn1->w == hSrcRgn2->w) &&
