...
static INLINE BYTE* neon_YuvToRgbPixel(BYTE* pRGB, int16x8_t Y, int16x8_t D, int16x8_t E, const uint8_t rPos,
...
static INLINE pstatus_t neon_YUV420ToX(const BYTE* pSrc[3], const UINT32 srcStep[3], BYTE* pDst, UINT32 dstStep,
...
static pstatus_t neon_YUV420ToRGB_8u_P3AC4R(const BYTE* pSrc[3], const UINT32 srcStep[3], BYTE* pDst, UINT32 dstStep,
...
static INLINE pstatus_t neon_YUV444ToX(const BYTE* pSrc[3], const UINT32 srcStep[3], BYTE* pDst, UINT32 dstStep,
                                       const prim_size_t* roi, const uint8_t rPos, const uint8_t gPos,
                                       const uint8_t bPos, const uint8_t aPos)
{
	UINT32 y;
...
static pstatus_t neon_YUV444ToRGB_8u_P3AC4R(const BYTE* pSrc[3], const UINT32 srcStep[3], BYTE* pDst, UINT32 dstStep,
                                            UINT32 DstFormat, const prim_size_t* roi)
{
	switch (DstFormat)
	{
...
static pstatus_t neon_LumaToYUV444(const BYTE* pSrcRaw[3], const UINT32 srcStep[3], BYTE* pDstRaw[3],
		...
		{
			{
				...
				const int16x8_t us = vreinterpretq_s16_u16(vaddw_u8(usub, u.val[1])); /* U2x1,2y + U2x,2y1 + U2x1,2y1 */
			...
			}
			{
				...
				const int16x8_t vs = vreinterpretq_s16_u16(vaddw_u8(vsub, v.val[1])); /* V2x1,2y + V2x,2y1 + V2x1,2y1 */
...
static pstatus_t neon_ChromaV1ToYUV444(const BYTE* pSrcRaw[3], const UINT32 srcStep[3], BYTE* pDstRaw[3],
                                       const UINT32 dstStep[3], const RECTANGLE_16* roi)
{
	const UINT32 mod = 16;
	UINT32 uY = 0;
...
static pstatus_t neon_ChromaV2ToYUV444(const BYTE* pSrc[3], const UINT32 srcStep[3], UINT32 nTotalWidth,
...
static pstatus_t neon_YUV420CombineToYUV444(avc444_frame_type type, const BYTE* pSrc[3], const UINT32 srcStep[3],
