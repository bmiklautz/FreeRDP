...
static BOOL memory_regions_overlap_2d(const BYTE* p1, int p1Step, int p1Size, const BYTE* p2, int p2Step, int p2Size,
 ...
 */
static pstatus_t general_copy_8u_AC4r(const BYTE* pSrc, INT32 srcStep, BYTE* pDst, INT32 dstStep, INT32 width,
                                      INT32 height)
{
	...
	if (memory_regions_overlap_2d(pSrc, srcStep, sizeof(UINT32), pDst, dstStep, sizeof(UINT32), width, height))
	{
		do
		{
...
static pstatus_t ippiCopy_8u_AC4r(const BYTE* pSrc, INT32 srcStep, BYTE* pDst, INT32 dstStep, INT32 width, INT32 height)
{
	IppiSize roi;
	roi.width = width;
	roi.height = height;
