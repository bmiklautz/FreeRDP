...
static BOOL similarRGB(const BYTE* src, const BYTE* dst, size_t size, UINT32 format)
{
	...
	for (x = 0; x < size; x++)
	{
		...
		{
			fprintf(stderr, "Color value  mismatch R[%02X %02X], G[%02X %02X], B[%02X %02X] at position %lu", sR, dR,
			        sG, dG, sA, dA, x);
			return FALSE;
		}
...
static BOOL TestPrimitiveYUVCombine(primitives_t* prims, prim_size_t roi)
{
	...
	if (prims->YUV420CombineToYUV444(AVC444_LUMA, (const BYTE**) luma, lumaStride, roi.width, roi.height, yuv,
	...
	if (prims->YUV420CombineToYUV444(AVC444_CHROMAv1, (const BYTE**) chroma, chromaStride, roi.width, roi.height, yuv,
	                                 yuvStride, &rect) != PRIMITIVES_SUCCESS)
	{
		PROFILER_EXIT(yuvCombine)
		goto fail;
	...
	if (prims->YUV444SplitToYUV420((const BYTE**) yuv, yuvStride, pmain, lumaStride, paux, chromaStride, &roi) !=
	    PRIMITIVES_SUCCESS)
	{
...
static BOOL TestPrimitiveYUV(primitives_t* prims, prim_size_t roi, BOOL use444)
{
	...
	UINT32 stride;
	const UINT32 formats[] = { PIXEL_FORMAT_XRGB32, PIXEL_FORMAT_XBGR32, PIXEL_FORMAT_ARGB32, PIXEL_FORMAT_ABGR32,
	...
	for (x = 0; x < sizeof(formats) / sizeof(formats[0]); x++)
	{
		...
		if ((!check_padding(yuv[0], size, padding, "Y")) || (!check_padding(yuv[1], uvsize, padding, "U")) ||
		...
		if (use444)
		{
			...
			rc = prims->YUV444ToRGB_8u_P3AC4R((const BYTE**) yuv, yuv_step, rgb_dst, stride, DstFormat, &roi);
		...
		else
		{
			...
			if (prims->YUV420ToRGB_8u_P3AC4R((const BYTE**) yuv, yuv_step, rgb_dst, stride, DstFormat, &roi) !=
			    PRIMITIVES_SUCCESS)
			{
...
static BOOL TestPrimitiveRgbToLumaChroma(primitives_t* prims, prim_size_t roi, UINT32 version)
{
	...
	for (x = 0; x < sizeof(formats) / sizeof(formats[0]); x++)
	{
		...
		if (!check_yuv420(luma, awidth, aheight, padding) || !check_yuv420(chroma, awidth, aheight, padding))
		{
			rc = -1;
			goto loop_fail;
		}
