...
static D3D_FEATURE_LEVEL FeatureLevels[] = { D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_10_0,
...
typedef HRESULT(WINAPI* fnD3D11CreateDevice)(IDXGIAdapter* pAdapter, D3D_DRIVER_TYPE DriverType, HMODULE Software,
...
DEFINE_GUID(IID_ID3D11VideoProcessorEnumerator, 0x31627037, 0x53AB, 0x4200, 0x90, 0x61, 0x05, 0xFA, 0xA9, 0xAB, 0x45,
            0xF9);
...
DEFINE_GUID(IID_ID3D11VideoDecoderOutputView, 0xC2931AEA, 0x2A85, 0x4f20, 0x86, 0x0F, 0xFB, 0xA1, 0xFD, 0x25, 0x6E,
            0x18);
DEFINE_GUID(IID_ID3D11VideoProcessorInputView, 0x11EC5A5F, 0x51DC, 0x4945, 0xAB, 0x34, 0x6E, 0x8C, 0x21, 0x30, 0x0E,
            0xA5);
DEFINE_GUID(IID_ID3D11VideoProcessorOutputView, 0xA048285E, 0x25A9, 0x4527, 0xBD, 0x93, 0xD6, 0x8B, 0x68, 0xC4, 0x42,
            0x54);
...
int win_shadow_dxgi_init_duplication(winShadowSubsystem* subsystem)
{
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "ID3D11Device::QueryInterface(IDXGIDevice) failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIDevice::GetParent(IDXGIAdapter) failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIAdapter::EnumOutputs failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIOutput::QueryInterface(IDXGIOutput1) failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIOutput1::DuplicateOutput failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "ID3D11Device::CreateTexture2D failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
		return -1;
	}
...
int win_shadow_dxgi_init(winShadowSubsystem* subsystem)
{
	...
	{
		hr = pfnD3D11CreateDevice(NULL, DriverTypes[DriverTypeIndex], NULL, 0, FeatureLevels, NumFeatureLevels,
		                          D3D11_SDK_VERSION, &(subsystem->dxgiDevice), &(subsystem->featureLevel),
                                     ...
                                     int width, int height)
{
	...
	subsystem->dxgiDeviceContext->lpVtbl->CopySubresourceRegion(subsystem->dxgiDeviceContext,
	                                                            (ID3D11Resource*) subsystem->dxgiStage, 0, 0, 0, 0,
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "ID3D11Texture2D::QueryInterface(IDXGISurface) failure: %s 0x%08lX", GetDxgiErrorString(hr), hr);
		return -1;
	}
...
int win_shadow_dxgi_get_next_frame(winShadowSubsystem* subsystem)
{
	...
	hr = subsystem->dxgiOutputDuplication->lpVtbl->AcquireNextFrame(
	  subsystem->dxgiOutputDuplication, timeout, &(subsystem->dxgiFrameInfo), &(subsystem->dxgiResource));
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIOutputDuplication::AcquireNextFrame failure: %s (0x%08lX)", GetDxgiErrorString(hr), hr);
	...
	hr = subsystem->dxgiResource->lpVtbl->QueryInterface(subsystem->dxgiResource, &IID_ID3D11Texture2D,
	...
	if (FAILED(hr))
	{
		WLog_ERR(TAG, "IDXGIResource::QueryInterface(ID3D11Texture2D) failure: %s (0x%08lX)", GetDxgiErrorString(hr),
		         hr);
		return -1;
	}
...
int win_shadow_dxgi_get_invalid_region(winShadowSubsystem* subsystem)
{
	...
	hr = subsystem->dxgiOutputDuplication->lpVtbl->GetFrameMoveRects(
	  subsystem->dxgiOutputDuplication, MoveRectsBufferSize, pMoveRectBuffer, &MoveRectsBufferSize);
	...
	hr = subsystem->dxgiOutputDuplication->lpVtbl->GetFrameDirtyRects(
	  subsystem->dxgiOutputDuplication, DirtyRectsBufferSize, pDirtyRectsBuffer, &DirtyRectsBufferSize);
