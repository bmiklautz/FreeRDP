...
static int x11_shadow_pam_conv(int num_msg, const struct pam_message** msg, struct pam_response** resp,
                               void* appdata_ptr)
{
	int index;
...
static int x11_shadow_pam_authenticate(rdpShadowSubsystem* subsystem, rdpShadowClient* client, const char* user,
...
static BOOL x11_shadow_input_keyboard_event(rdpShadowSubsystem* subsystem, rdpShadowClient* client, UINT16 flags,
                                            UINT16 code)
{
#ifdef WITH_XTEST
...
static BOOL x11_shadow_input_unicode_keyboard_event(rdpShadowSubsystem* subsystem, rdpShadowClient* client,
                                                    UINT16 flags, UINT16 code)
{
	return TRUE;
}

static BOOL x11_shadow_input_mouse_event(rdpShadowSubsystem* subsystem, rdpShadowClient* client, UINT16 flags, UINT16 x,
                                         UINT16 y)
{
#ifdef WITH_XTEST
	x11ShadowSubsystem* x11 = (x11ShadowSubsystem*) subsystem;
...
static BOOL x11_shadow_input_extended_mouse_event(rdpShadowSubsystem* subsystem, rdpShadowClient* client, UINT16 flags,
                                                  UINT16 x, UINT16 y)
{
#ifdef WITH_XTEST
	x11ShadowSubsystem* x11 = (x11ShadowSubsystem*) subsystem;
...
static int x11_shadow_pointer_alpha_update(x11ShadowSubsystem* subsystem)
{
	...
	if (shadow_subsystem_pointer_convert_alpha_pointer_data(subsystem->cursorPixels, TRUE, msg->width, msg->height,
	                                                        msg) < 0)
	{
		free(msg);
		return -1;
	...
	return shadow_client_boardcast_msg(subsystem->common.server, NULL, msgId, (SHADOW_MSG_OUT*) msg, NULL) ? 1 : -1;
}
...
static int x11_shadow_query_cursor(x11ShadowSubsystem* subsystem, BOOL getImage)
{
	...
	else
	{
		...
		if (!XQueryPointer(subsystem->display, subsystem->root_window, &root, &child, &root_x, &root_y, &win_x, &win_y,
		                   &mask))
		{
			XUnlockDisplay(subsystem->display);
			return -1;
...
static int x11_shadow_error_handler_for_capture(Display* display, XErrorEvent* event)
{
	...
	WLog_ERR(TAG, "X11 error: %s Error code: %x, request code: %x, minor code: %x", msg, event->error_code,
...
static int x11_shadow_screen_grab(x11ShadowSubsystem* subsystem)
{
	...
	if (subsystem->use_xshm)
	{
		...
		XCopyArea(subsystem->display, subsystem->root_window, subsystem->fb_pixmap, subsystem->xshm_gc, 0, 0,
		          subsystem->width, subsystem->height, 0, 0);
	...
	{
		image = XGetImage(subsystem->display, subsystem->root_window, surface->x, surface->y, surface->width,
	...
	if (status)
	{
		...
		if (!region16_is_empty(&(surface->invalidRegion)))
		{
			...
			if (!freerdp_image_copy(surface->data, surface->format, surface->scanline, x, y, width, height,
			                        (BYTE*) image->data, PIXEL_FORMAT_BGRX32, image->bytes_per_line, x, y, NULL,
			                        FREERDP_FLIP_NONE))
				goto fail_capture;
...
static int x11_shadow_xfixes_init(x11ShadowSubsystem* subsystem)
{
	...
	XFixesSelectCursorInput(subsystem->display, subsystem->root_window, XFixesDisplayCursorNotifyMask);
	return 1;
#else
	return -1;
#endif
...
static int x11_shadow_xshm_init(x11ShadowSubsystem* subsystem)
{
	...
	subsystem->fb_image = XShmCreateImage(subsystem->display, subsystem->visual, subsystem->depth, ZPixmap, NULL,
	...
	subsystem->fb_shm_info.shmid =
	  shmget(IPC_PRIVATE, subsystem->fb_image->bytes_per_line * subsystem->fb_image->height, IPC_CREAT | 0600);
	...
	subsystem->fb_pixmap = XShmCreatePixmap(subsystem->display, subsystem->root_window, subsystem->fb_image->data,
	...
	subsystem->xshm_gc =
	  XCreateGC(subsystem->display, subsystem->root_window, GCSubwindowMode | GCGraphicsExposures, &values);
...
static int x11_shadow_subsystem_init(rdpShadowSubsystem* sub)
{
	...
	if (!(subsystem->common.event = CreateFileDescriptorEvent(NULL, FALSE, FALSE, subsystem->xfds, WINPR_FD_READ)))
		return -1;
...
static int x11_shadow_subsystem_start(rdpShadowSubsystem* sub)
{
	...
	if (!(subsystem->thread = CreateThread(NULL, 0, x11_shadow_subsystem_thread, (void*) subsystem, 0, NULL)))
	{
