...
static INLINE void Stream_Seek(wStream* s, size_t _offset) { s->pointer += (_offset); }

static INLINE void Stream_Rewind(wStream* s, size_t _offset) { s->pointer -= (_offset); }
...
#define _stream_read_n32_le(_t, _s, _v, _p)                                                                            \
	do                                                                                                                 \
	{                                                                                                                  \
		_v = (_t)(*_s->pointer) + (((_t)(*(_s->pointer + 1))) << 8) + (((_t)(*(_s->pointer + 2))) << 16) +             \
		  (((_t)(*(_s->pointer + 3))) << 24);                                                                          \
		if (_p)                                                                                                        \
			Stream_Seek(_s, sizeof(_t));                                                                               \
	} while (0)
...
#define _stream_read_n32_be(_t, _s, _v, _p)                                                                            \
	do                                                                                                                 \
	{                                                                                                                  \
		_v = (((_t)(*(_s->pointer))) << 24) + (((_t)(*(_s->pointer + 1))) << 16) + (((_t)(*(_s->pointer + 2))) << 8) + \
		  (((_t)(*(_s->pointer + 3))));                                                                                \
		if (_p)                                                                                                        \
			Stream_Seek(_s, sizeof(_t));                                                                               \
	} while (0)
...
#define _stream_read_n64_le(_t, _s, _v, _p)                                                                            \
	...
	{                                                                                                                  \
		_v = (_t)(*_s->pointer) + (((_t)(*(_s->pointer + 1))) << 8) + (((_t)(*(_s->pointer + 2))) << 16) +             \
		  (((_t)(*(_s->pointer + 3))) << 24) + (((_t)(*(_s->pointer + 4))) << 32) +                                    \
		  (((_t)(*(_s->pointer + 5))) << 40) + (((_t)(*(_s->pointer + 6))) << 48) +                                    \
		  (((_t)(*(_s->pointer + 7))) << 56);                                                                          \
		if (_p)                                                                                                        \
			Stream_Seek(_s, sizeof(_t));                                                                               \
	} while (0)
...
static INLINE void Stream_Peek(wStream* _s, void* _b, size_t _n) { memcpy(_b, (_s->pointer), (_n)); }

static INLINE void Stream_Write_UINT8(wStream* _s, UINT8 _v) { *_s->pointer++ = (UINT8)(_v); }
...
static INLINE BYTE* Stream_Buffer(wStream* _s) { return _s->buffer; }

#define Stream_GetBuffer(_s, _b) _b = Stream_Buffer(_s)
static INLINE void Stream_SetBuffer(wStream* _s, BYTE* _b) { _s->buffer = _b; }

static INLINE BYTE* Stream_Pointer(wStream* _s) { return _s->pointer; }

#define Stream_GetPointer(_s, _p) _p = Stream_Pointer(_s)
static INLINE void Stream_SetPointer(wStream* _s, BYTE* _p) { _s->pointer = _p; }

static INLINE size_t Stream_Length(wStream* _s) { return _s->length; }

#define Stream_GetLength(_s, _l) _l = Stream_Length(_s)
static INLINE void Stream_SetLength(wStream* _s, size_t _l) { _s->length = _l; }

static INLINE size_t Stream_Capacity(wStream* _s) { return _s->capacity; }

#define Stream_GetCapacity(_s, _c) _c = Stream_Capacity(_s);
static INLINE void Stream_SetCapacity(wStream* _s, size_t _c) { _s->capacity = _c; }

static INLINE size_t Stream_GetPosition(wStream* _s) { return (_s->pointer - _s->buffer); }

static INLINE void Stream_SetPosition(wStream* _s, size_t _p) { _s->pointer = _s->buffer + (_p); }

static INLINE void Stream_SealLength(wStream* _s) { _s->length = (_s->pointer - _s->buffer); }

static INLINE size_t Stream_GetRemainingCapacity(wStream* _s) { return (_s->capacity - (_s->pointer - _s->buffer)); }

static INLINE size_t Stream_GetRemainingLength(wStream* _s) { return (_s->length - (_s->pointer - _s->buffer)); }

static INLINE void Stream_Clear(wStream* _s) { memset(_s->buffer, 0, _s->capacity); }
