 ...
 *   ERROR_BAD_DEVICE
 */
BOOL CommReadFile(HANDLE hDevice, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead,
                  LPOVERLAPPED lpOverlapped)
{
	...
	*lpNumberOfBytesRead = 0; /* will be ajusted if required ... */
	 ...
	 *         0            |            0               |           0              |   N  |   0   | INDEF | Blocks for
	 * N bytes available. 0< Ti <MAXULONG	|            0               |           0              |   N  |   Ti  |
	 * INDEF | Blocks on first byte, then use Ti between bytes. MAXULONG       |            0               | 0 |   0  |
	 * 0   |   0   | Returns immediately with bytes available (don't block) MAXULONG       |         MAXULONG |      0<
	 * Tc <MAXULONG     |   N  |   0   |   Tc  | Blocks on first byte during Tc or returns immediately whith bytes
	 * available MAXULONG       |            m               |        MAXULONG          |                      | Invalid
	 *         0            |            m               |      0< Tc <MAXULONG     |   N  |   0   |  Tmax | Blocks on
	 * first byte during Tmax or returns immediately whith bytes available 0< Ti <MAXULONG    |            m |      0<
	 * Tc <MAXULONG     |   N  |   Ti  |  Tmax | Blocks on first byte, then use Ti between bytes. Tmax is used for the
	 * whole system call.
	 */
	...
	if ((pTimeouts->ReadIntervalTimeout == MAXULONG) && (pTimeouts->ReadTotalTimeoutMultiplier == 0) &&
	...
	if (FD_ISSET(pComm->fd_read, &read_set))
	{
		...
		{
			CommLog_Print(WLOG_WARN,
			              "CommReadFile failed, ReadIntervalTimeout=%" PRIu32 ", ReadTotalTimeoutMultiplier=%" PRIu32
			              ", ReadTotalTimeoutConstant=%" PRIu32 " VMIN=%u, VTIME=%u",
			              pTimeouts->ReadIntervalTimeout, pTimeouts->ReadTotalTimeoutMultiplier,
			              pTimeouts->ReadTotalTimeoutConstant, currentTermios.c_cc[VMIN], currentTermios.c_cc[VTIME]);
			CommLog_Print(WLOG_WARN, "CommReadFile failed, nNumberOfBytesToRead=%" PRIu32 ", errno=[%d] %s",
 ...
 *   ERROR_BAD_DEVICE
 */
BOOL CommWriteFile(HANDLE hDevice, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten,
                   LPOVERLAPPED lpOverlapped)
{
	WINPR_COMM* pComm = (WINPR_COMM*) hDevice;
	struct timeval tmaxTimeout, *pTmaxTimeout;
	...
	*lpNumberOfBytesWritten = 0; /* will be ajusted if required ... */
	...
	/* ms */
	ULONGLONG Tmax =
	  nNumberOfBytesToWrite * pComm->timeouts.WriteTotalTimeoutMultiplier + pComm->timeouts.WriteTotalTimeoutConstant;
	...
	while (*lpNumberOfBytesWritten < nNumberOfBytesToWrite)
	{
		...
		if (FD_ISSET(pComm->fd_write, &write_set))
		{
			...
			if (nbWritten < 0)
			{
				CommLog_Print(WLOG_WARN, "CommWriteFile failed after %" PRIu32 " bytes written, errno=[%d] %s\n",
