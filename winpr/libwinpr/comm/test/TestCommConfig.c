...
int TestCommConfig(int argc, char* argv[])
{
	...
	hComm = CreateFileA(lpFileName, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, /* invalid parmaeter */
	...
	{
		fprintf(stderr,
		        "Got an unexpeted value among: BaudRate: %" PRIu32 " ByteSize: %" PRIu8 " Parity: %" PRIu8
		        " StopBits: %" PRIu8 "\n",
		        dcb.BaudRate, dcb.ByteSize, dcb.Parity, dcb.StopBits);
	}
