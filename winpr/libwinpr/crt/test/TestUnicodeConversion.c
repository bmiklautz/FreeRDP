...
int convert_utf16_to_utf8(BYTE* lpWideCharStr, BYTE* expected_lpMultiByteStr, int expected_cbMultiByte)
{
	...
	length = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR) lpWideCharStr, cchWideChar + 1, lpMultiByteStr, cbMultiByte,
	                             NULL, NULL);
...
{
	BYTE src1[] = "\x52\x00\x49\x00\x43\x00\x48\x00\x20\x00\x54\x00\x45\x00\x58\x00\x54\x00\x20\x00\x46\x00\x4f\x00\x52"
	              "\x00\x4d\x00\x41\x00\x54\x00\x40\x00\x40\x00\x40\x00";
	BYTE src2[] = "\x52\x00\x49\x00\x43\x00\x48\x00\x20\x00\x54\x00\x45\x00\x58\x00\x54\x00\x20\x00\x46\x00\x4f\x00\x52"
	              "\x00\x4d\x00\x41\x00\x54\x00\x00\x00";
...
BOOL test_ConvertToUnicode_wrapper()
{
	...
	BYTE cmp0[] = "\x52\x00\x49\x00\x43\x00\x48\x00\x20\x00\x54\x00\x45\x00\x58\x00\x54\x00\x20\x00\x46\x00\x4f\x00\x52"
	              "\x00\x4d\x00\x41\x00\x54\x00\x00\x00";
	WCHAR* dst = NULL;
	int i;
...
int TestUnicodeConversion(int argc, char* argv[])
{
	...
	/*
	   ...
	   ConvertFromUnicode(CP_UTF8, 0, (WCHAR*) src, 16, &dst, 0, NULL, NULL); printf("ConvertFromUnicode returned %d
	   dst=[%s]\n", num, dst); string_hexdump((BYTE*)dst, num+1);
	    }
	    if (1)
	    {
