...
static char* GetPath_XDG_CACHE_HOME(void)
{
	 ...
	 *
	 * $XDG_CACHE_HOME defines the base directory relative to which user specific non-essential data files should be
	 * stored. If $XDG_CACHE_HOME is either not set or empty, a default equal to $HOME/.cache should be used.
	 */
...
char* GetPath_XDG_RUNTIME_DIR(void)
{
...
#else
	/**
	 * There is a single base directory relative to which user-specific runtime files and other file objects should be
	 * placed. This directory is defined by the environment variable $XDG_RUNTIME_DIR.
	 *
	 ...
	 * first login to his last logout on the system, and not removed in between. Files in the directory MUST not survive
	 ...
	 * AF_UNIX sockets, symbolic links, hard links, proper permissions, file locking, sparse files, memory mapping, file
	 * change notifications, a reliable hard link count must be supported, and no restrictions on the file name
	 * character set should be imposed. Files in this directory MAY be subjected to periodic clean-up. To ensure that
	 * your files are not removed, they should have their access time timestamp modified at least once every 6 hours of
	 ...
	 * and print a warning message. Applications should use this directory for communication and synchronization
	 * purposes and should not place larger files in it, since it might reside in runtime memory and cannot necessarily
