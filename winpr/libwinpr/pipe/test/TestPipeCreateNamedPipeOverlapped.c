...
static DWORD WINAPI named_pipe_client_thread(LPVOID arg)
{
	...
	hNamedPipe =
	  CreateFile(lpszPipeName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
...
static DWORD WINAPI named_pipe_server_thread(LPVOID arg)
{
	...
	hNamedPipe = CreateNamedPipe(lpszPipeName, PIPE_ACCESS_DUPLEX | FILE_FLAG_OVERLAPPED,
