...
int TestPipeCreatePipe(int argc, char* argv[])
{
	...
	{
		_tprintf(_T("WriteFile: unexpected number of bytes written: Actual: %") _T(PRIu32) _T(", Expected: %") _T(
		           PRIuz) _T("\n"),
	...
	{
		_tprintf(
		  _T("ReadFile: unexpected number of bytes read: Actual: %") _T(PRIu32) _T(", Expected: %") _T(PRIuz) _T("\n"),
