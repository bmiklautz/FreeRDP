                                         ...
                                         UINT32* time_rec)
{
	...
	status = g_GssApi->gss_acquire_cred(minor_status, desired_name, time_req, desired_mechs, cred_usage,
	                                    output_cred_handle, actual_mechs, time_rec);
	WLog_DBG(TAG, "gss_acquire_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_release_cred(UINT32* minor_status, sspi_gss_cred_id_t* cred_handle)
{
	...
	WLog_DBG(TAG, "gss_release_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                             ...
                                             sspi_gss_buffer_t output_token, UINT32* ret_flags, UINT32* time_rec)
{
	...
	status = g_GssApi->gss_init_sec_context(minor_status, claimant_cred_handle, context_handle, target_name, mech_type,
	                                        req_flags, time_req, input_chan_bindings, input_token, actual_mech_type,
	                                        output_token, ret_flags, time_rec);
	WLog_DBG(TAG, "gss_init_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_cred_id_t* delegated_cred_handle)
{
	...
	status = g_GssApi->gss_accept_sec_context(minor_status, context_handle, acceptor_cred_handle, input_token_buffer,
	                                          input_chan_bindings, src_name, mech_type, output_token, ret_flags,
	                                          time_rec, delegated_cred_handle);
	WLog_DBG(TAG, "gss_accept_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                                  ...
                                                  sspi_gss_buffer_t token_buffer)
{
	...
	WLog_DBG(TAG, "gss_process_context_token: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_buffer_t output_token)
{
	...
	WLog_DBG(TAG, "gss_delete_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_context_time(UINT32* minor_status, sspi_gss_ctx_id_t context_handle, UINT32* time_rec)
{
	...
	WLog_DBG(TAG, "gss_context_time: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                 ...
                                 sspi_gss_buffer_t output_message_buffer)
{
	...
	status = g_GssApi->gss_wrap(minor_status, context_handle, conf_req_flag, qop_req, input_message_buffer, conf_state,
                                   ...
                                   sspi_gss_buffer_t output_message_buffer, int* conf_state, sspi_gss_qop_t* qop_state)
{
	...
	status = g_GssApi->gss_unwrap(minor_status, context_handle, input_message_buffer, output_message_buffer, conf_state,
	                              qop_state);
                                           ...
                                           sspi_gss_buffer_t status_string)
{
	...
	status =
	  g_GssApi->gss_display_status(minor_status, status_value, status_type, mech_type, message_context, status_string);
	WLog_DBG(TAG, "gss_display_status: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_indicate_mechs(UINT32* minor_status, sspi_gss_OID_set* mech_set)
{
	...
	WLog_DBG(TAG, "gss_indicate_mechs: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                         ...
                                         int* name_equal)
{
	...
	WLog_DBG(TAG, "gss_compare_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                         ...
                                         sspi_gss_buffer_t output_name_buffer, sspi_gss_OID* output_name_type)
{
	...
	WLog_DBG(TAG, "gss_display_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_release_name(UINT32* minor_status, sspi_gss_name_t* input_name)
{
	...
	WLog_DBG(TAG, "gss_release_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_release_buffer(UINT32* minor_status, sspi_gss_buffer_t buffer)
{
	...
	WLog_DBG(TAG, "gss_release_buffer: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_release_oid_set(UINT32* minor_status, sspi_gss_OID_set* set)
{
	...
	WLog_DBG(TAG, "gss_release_oid_set: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                         ...
                                         sspi_gss_OID_set* mechanisms)
{
	...
	WLog_DBG(TAG, "gss_inquire_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                            ...
                                            int* open)
{
	...
	status = g_GssApi->gss_inquire_context(minor_status, context_handle, src_name, targ_name, lifetime_rec, mech_type,
	                                       ctx_flags, locally_initiated, open);
	WLog_DBG(TAG, "gss_inquire_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                            ...
                                            sspi_gss_qop_t qop_req, UINT32 req_output_size, UINT32* max_input_size)
{
	...
	status = g_GssApi->gss_wrap_size_limit(minor_status, context_handle, conf_req_flag, qop_req, req_output_size,
	                                       max_input_size);
	WLog_DBG(TAG, "gss_wrap_size_limit: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_name_t* output_name)
{
	...
	WLog_DBG(TAG, "gss_import_name_object: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_OID desired_name_type, void** output_name)
{
	...
	WLog_DBG(TAG, "gss_export_name_object: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                     ...
                                     UINT32* acceptor_time_rec)
{
	...
	status = g_GssApi->gss_add_cred(minor_status, input_cred_handle, desired_name, desired_mech, cred_usage,
	                                initiator_time_req, acceptor_time_req, output_cred_handle, actual_mechs,
                                                 ...
                                                 sspi_gss_cred_usage_t* cred_usage)
{
	...
	status = g_GssApi->gss_inquire_cred_by_mech(minor_status, cred_handle, mech_type, name, initiator_lifetime,
	                                            acceptor_lifetime, cred_usage);
	WLog_DBG(TAG, "gss_inquire_cred_by_mech: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_buffer_t interprocess_token)
{
	...
	WLog_DBG(TAG, "gss_export_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                               ...
                                               sspi_gss_ctx_id_t* context_handle)
{
	...
	WLog_DBG(TAG, "gss_import_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_create_empty_oid_set(UINT32* minor_status, sspi_gss_OID_set* oid_set)
{
	...
	WLog_DBG(TAG, "gss_create_empty_oid_set: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_add_oid_set_member(UINT32* minor_status, sspi_gss_OID member_oid, sspi_gss_OID_set* oid_set)
{
	...
	WLog_DBG(TAG, "gss_add_oid_set_member: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                                ...
                                                int* present)
{
	...
	WLog_DBG(TAG, "gss_test_oid_set_member: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                                   ...
                                                   sspi_gss_OID_set* name_types)
{
	...
	WLog_DBG(TAG, "gss_inquire_names_for_mech: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                                   ...
                                                   sspi_gss_OID_set* mech_types)
{
	...
	WLog_DBG(TAG, "gss_inquire_mechs_for_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                 ...
                                 sspi_gss_buffer_t output_message_buffer)
{
	...
	status = g_GssApi->gss_seal(minor_status, context_handle, conf_req_flag, qop_req, input_message_buffer, conf_state,
	                            output_message_buffer);
                                   ...
                                   int* conf_state, int* qop_state)
{
	...
	status = g_GssApi->gss_unseal(minor_status, context_handle, input_message_buffer, output_message_buffer, conf_state,
	                              qop_state);
                                           ...
                                           sspi_gss_name_t* dest_name)
{
	...
	WLog_DBG(TAG, "gss_duplicate_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                              ...
                                              const sspi_gss_OID mech_type, sspi_gss_name_t* output_name)
{
	...
	WLog_DBG(TAG, "gss_canonicalize_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                          ...
                                          sspi_gss_buffer_t prf_out)
{
	...
	status = g_GssApi->gss_pseudo_random(minor_status, context, prf_key, prf_in, desired_output_len, prf_out);
	WLog_DBG(TAG, "gss_pseudo_random: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                       ...
                                       sspi_gss_cred_usage_t* cred_usage_stored)
{
	...
	status = g_GssApi->gss_store_cred(minor_status, input_cred_handle, input_usage, desired_mech, overwrite_cred,
	                                  default_cred, elements_stored, cred_usage_stored);
                                          ...
                                          const sspi_gss_OID_set mech_set)
{
	...
	WLog_DBG(TAG, "gss_set_neg_mechs: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
