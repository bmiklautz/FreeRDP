...
VOID CALLBACK TimerRoutine(PVOID lpParam, BOOLEAN TimerOrWaitFired)
{
	...
	printf("TimerRoutine: TimerId: %" PRIu32 " FireCount: %" PRIu32 " ActualTime: %" PRIu32 " ExpectedTime: %" PRIu32
	       " Discrepancy: %" PRIu32 "\n",
...
int TestSynchTimerQueue(int argc, char* argv[])
{
	...
	for (index = 0; index < TIMER_COUNT; index++)
	{
		...
		if (!CreateTimerQueueTimer(&hTimers[index], hTimerQueue, (WAITORTIMERCALLBACK) TimerRoutine, &apcData[index],
