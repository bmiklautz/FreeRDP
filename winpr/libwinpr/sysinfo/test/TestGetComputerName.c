...
BOOL Test_GetComputerName(void)
{
	...
	/* check if returned size is valid: must be the size of the buffer required, including the terminating null
...
BOOL Test_GetComputerNameEx_Format(COMPUTER_NAME_FORMAT format)
{
	 ...
	 * To ensure that this buffer is large enough, set this parameter to NULL and use the required buffer size returned
	 * in the lpnSize parameter.
	 *
	 * lpnSize [in, out]
	 * On input, specifies the size of the buffer, in TCHARs.
	 * On output, receives the number of TCHARs copied to the destination buffer, not including the terminating null
	 * character. If the buffer is too small, the function fails and GetLastError returns ERROR_MORE_DATA. This
	 * parameter receives the size of the buffer required, including the terminating null character. If lpBuffer is
