 ...
 */
TRIO_PRIVATE int TrioParseQualifiers TRIO_ARGS4((type, format, offset, parameter), int type, TRIO_CONST char* format,
                                                int offset, trio_parameter_t* parameter)
{
	char ch;
 ...
 */
TRIO_PRIVATE int TrioParseSpecifier TRIO_ARGS4((type, format, offset, parameter), int type, TRIO_CONST char* format,
                                               int offset, trio_parameter_t* parameter)
{
 ...
 */
TRIO_PRIVATE int TrioParse TRIO_ARGS6((type, format, parameters, arglist, argfunc, argarray), int type,
                                      TRIO_CONST char* format, trio_parameter_t* parameters, va_list arglist,
                                      trio_argfunc_t argfunc, trio_pointer_t* argarray)
{
	...
	for (num = 0; num <= maxParam; num++)
	{
		...
		switch (parameters[i].type)
		{
			...
			case FORMAT_COUNT:
			case FORMAT_UNKNOWN:
				parameters[i].data.pointer = (argfunc == NULL) ? va_arg(arglist, trio_pointer_t)
				...
				if (TYPE_SCAN == type)
				{
					...
					else
					{
						if (parameters[i].type == FORMAT_CHAR)
							...
							parameters[i].data.pointer = (trio_pointer_t)((int*) argfunc(argarray, num, TRIO_TYPE_INT));
					}
				}
				else
 ...
 */
TRIO_PRIVATE void TrioWriteNumber TRIO_ARGS6((self, number, flags, width, precision, base), trio_class_t* self,
                                             trio_uintmax_t number, trio_flags_t flags, int width, int precision,
                                             int base)
{
 ...
 *  Output a string
 */
TRIO_PRIVATE void TrioWriteString TRIO_ARGS5((self, string, flags, width, precision), trio_class_t* self,
                                             TRIO_CONST char* string, trio_flags_t flags, int width, int precision)
{
	int length;
	int ch;
 ...
 */
#if TRIO_FEATURE_WIDECHAR
TRIO_PRIVATE int TrioWriteWideStringCharacter TRIO_ARGS4((self, wch, flags, width), trio_class_t* self,
                                                         trio_wchar_t wch, trio_flags_t flags, int width)
{
	int size;
	int i;
	int ch;
 ...
 */
#if TRIO_FEATURE_WIDECHAR
TRIO_PRIVATE void TrioWriteWideString TRIO_ARGS5((self, wstring, flags, width, precision), trio_class_t* self,
                                                 TRIO_CONST trio_wchar_t* wstring, trio_flags_t flags, int width,
                                                 int precision)
{
	int length;
	int size;
 ...
 */
#if TRIO_FEATURE_FLOAT
TRIO_PRIVATE void TrioWriteDouble TRIO_ARGS6((self, number, flags, width, precision, base), trio_class_t* self,
                                             trio_long_double_t number, trio_flags_t flags, int width, int precision,
                                             int base)
{
	...
	switch (trio_fpclassify_and_signbit(number, &isNegative))
	{
		...
		case TRIO_FP_INFINITE:
			...
			{
				/* Negative infinity */
				TrioWriteString(self, (flags & FLAGS_UPPER) ? "-" INFINITE_UPPER : "-" INFINITE_LOWER, flags, width,
				                precision);
				return;
			}
			else
...
reprocess:
	...
	{
		exponentDigits =
		  (uExponent == 0) ? 1 : (int) trio_ceil(TrioLogarithm((double) (uExponent + 1), (isHex) ? 10 : base));
	}
 ...
 */
TRIO_PRIVATE int TrioFormatProcess TRIO_ARGS3((data, format, parameters), trio_class_t* data, TRIO_CONST char* format,
	...
	for (;;)
	{
		...
		switch (parameters[i].type)
		{
			case FORMAT_CHAR:
				...
				{
					TrioWriteWideStringCharacter(data, (trio_wchar_t) parameters[i].data.number.as_signed, flags,
					                             NO_WIDTH);
				}
				else
#endif
			...
			case FORMAT_ERRNO:
				...
				{
					data->OutStream(data, '#');
					TrioWriteNumber(data, (trio_uintmax_t) parameters[i].data.errorNumber, flags, width, precision,
					                BASE_DECIMAL);
				}
				break; /* FORMAT_ERRNO */
 ...
 */
#if TRIO_EXTENSION
TRIO_PRIVATE int TrioFormatRef TRIO_ARGS5((reference, format, arglist, argfunc, argarray), trio_reference_t* reference,
                                          TRIO_CONST char* format, va_list arglist, trio_argfunc_t argfunc,
...
TRIO_PRIVATE int TrioFormat TRIO_ARGS7((destination, destinationSize, OutStream, format, arglist, argfunc, argarray),
                                       ...
                                       va_list arglist, trio_argfunc_t argfunc, trio_pointer_t* argarray)
{
	int status;
	trio_class_t data;
	trio_parameter_t parameters[MAX_PARAMETERS];
 ...
 */
#if TRIO_FEATURE_CLOSURE
TRIO_PUBLIC int trio_cprintf TRIO_VARGS4((stream, closure, format, va_alist), trio_outstream_t stream,
                                         trio_pointer_t closure, TRIO_CONST char* format, TRIO_VA_DECL)
{
	int status;
	va_list args;
	trio_custom_t data;
...
TRIO_PUBLIC int trio_vcprintf TRIO_ARGS4((stream, closure, format, args), trio_outstream_t stream,
                                         trio_pointer_t closure, TRIO_CONST char* format, va_list args)
{
	trio_custom_t data;
...
TRIO_PUBLIC int trio_cprintfv TRIO_ARGS4((stream, closure, format, args), trio_outstream_t stream,
                                         trio_pointer_t closure, TRIO_CONST char* format, trio_pointer_t* args)
{
	static va_list unused;
	trio_custom_t data;
...
TRIO_PUBLIC int trio_cprintff TRIO_ARGS5((stream, closure, format, argfunc, context), trio_outstream_t stream,
                                         trio_pointer_t closure, TRIO_CONST char* format, trio_argfunc_t argfunc,
...
/**
   ...
   @return Number of printed characters.
 */
TRIO_PUBLIC int trio_sprintf TRIO_VARGS3((buffer, format, va_alist), char* buffer, TRIO_CONST char* format,
                                         TRIO_VA_DECL)
{
	int status;
	va_list args;
...
/**
   ...
   @return Number of printed characters.
 */
TRIO_PUBLIC int trio_sprintfv TRIO_ARGS3((buffer, format, args), char* buffer, TRIO_CONST char* format,
                                         trio_pointer_t* args)
{
	static va_list unused;
	int status;
 ...
 */
#if TRIO_EXTENSION
TRIO_PUBLIC int trio_snprintfcat TRIO_VARGS4((buffer, max, format, va_alist), char* buffer, size_t max,
                                             TRIO_CONST char* format, TRIO_VA_DECL)
{
	int status;
	va_list args;
 ...
 */
#if TRIO_FEATURE_DYNAMICSTRING
TRIO_PUBLIC int trio_asprintf TRIO_VARGS3((result, format, va_alist), char** result, TRIO_CONST char* format,
                                          TRIO_VA_DECL)
{
	va_list args;
	int status;
 ...
 */
#if TRIO_FEATURE_DYNAMICSTRING
TRIO_PUBLIC int trio_asprintfv TRIO_ARGS3((result, format, args), char** result, TRIO_CONST char* format,
                                          trio_pointer_t* args)
{
	static va_list unused;
	int status;
 ...
 */
int trio_get_width TRIO_ARGS1((ref), trio_pointer_t ref) { return ((trio_reference_t*) ref)->parameter->width; }
 ...
 */
int trio_get_base TRIO_ARGS1((ref), trio_pointer_t ref) { return (((trio_reference_t*) ref)->parameter->base); }
...
void trio_print_int TRIO_ARGS2((ref, number), trio_pointer_t ref, int number)
{
	...
	TrioWriteNumber(self->data, (trio_uintmax_t) number, self->parameter->flags, self->parameter->width,
...
void trio_print_double TRIO_ARGS2((ref, number), trio_pointer_t ref, double number)
{
	...
	TrioWriteDouble(self->data, number, self->parameter->flags, self->parameter->width, self->parameter->precision,
	                self->parameter->base);
}
#endif /* TRIO_FEATURE_FLOAT */
 ...
 */
TRIO_PRIVATE BOOLEAN_T TrioReadNumber TRIO_ARGS5((self, target, flags, width, base), trio_class_t* self,
                                                 trio_uintmax_t* target, trio_flags_t flags, int width, int base)
{
 ...
 * TrioReadChar
 */
TRIO_PRIVATE int TrioReadChar TRIO_ARGS4((self, target, flags, width), trio_class_t* self, char* target,
 ...
 * TrioReadString
 */
TRIO_PRIVATE BOOLEAN_T TrioReadString TRIO_ARGS4((self, target, flags, width), trio_class_t* self, char* target,
                                                 trio_flags_t flags, int width)
{
	int i;
 ...
 */
#if TRIO_FEATURE_WIDECHAR
TRIO_PRIVATE int TrioReadWideChar TRIO_ARGS4((self, target, flags, width), trio_class_t* self, trio_wchar_t* target,
                                             trio_flags_t flags, int width)
{
	int i;
	int j;
 ...
 */
#if TRIO_FEATURE_WIDECHAR
TRIO_PRIVATE BOOLEAN_T TrioReadWideString TRIO_ARGS4((self, target, flags, width), trio_class_t* self,
 ...
 */
TRIO_PRIVATE BOOLEAN_T TrioReadGroup TRIO_ARGS5((self, target, characterclass, flags, width), trio_class_t* self,
                                                char* target, int* characterclass, trio_flags_t flags, int width)
{
	int ch;
	int i;
...
TRIO_PRIVATE BOOLEAN_T TrioReadDouble TRIO_ARGS4((self, target, flags, width), trio_class_t* self,
                                                 trio_pointer_t target, trio_flags_t flags, int width)
{
	int ch;
 ...
 * TrioReadPointer
 */
TRIO_PRIVATE BOOLEAN_T TrioReadPointer TRIO_ARGS3((self, target, flags), trio_class_t* self, trio_pointer_t* target,
                                                  trio_flags_t flags)
{
	trio_uintmax_t number;
 ...
 * TrioScanProcess
 */
TRIO_PRIVATE int TrioScanProcess TRIO_ARGS3((data, format, parameters), trio_class_t* data, TRIO_CONST char* format,
                                            trio_parameter_t* parameters)
{
	int status;
	int assignment;
	...
	for (;;)
	{
		...
		switch (parameters[i].type)
		{
			...
			case FORMAT_STRING:
				...
				if (flags & FLAGS_WIDECHAR)
				{
					if (!TrioReadWideString(data, (flags & FLAGS_IGNORE) ? NULL : parameters[i].data.wstring, flags,
					                        width))
					{
						status = assignment;
						goto end;
			...
			case FORMAT_DOUBLE:
			{
				...
				{
					pointer = (flags & FLAGS_LONGDOUBLE) ? (trio_pointer_t) parameters[i].data.longdoublePointer
			...
			case FORMAT_GROUP:
			{
				...
				if (!TrioReadGroup(data, (flags & FLAGS_IGNORE) ? NULL : parameters[i].data.string, characterclass,
				                   flags, parameters[i].width))
				{
					status = assignment;
					goto end;
				}
			...
			case FORMAT_CHAR:
				...
				if (flags & FLAGS_WIDECHAR)
				{
					if (TrioReadWideChar(data, (flags & FLAGS_IGNORE) ? NULL : parameters[i].data.wstring, flags,
...
#endif
				{
					if (TrioReadChar(data, (flags & FLAGS_IGNORE) ? NULL : parameters[i].data.string, flags,
					                 (width == NO_WIDTH) ? 1 : width) == 0)
					{
						status = assignment;
						goto end;
			...
			case FORMAT_POINTER:
				if (!TrioReadPointer(data, (flags & FLAGS_IGNORE) ? NULL : (trio_pointer_t*) parameters[i].data.pointer,
				                     flags))
				{
					status = assignment;
					goto end;
...
TRIO_PRIVATE int TrioScan TRIO_ARGS8((source, sourceSize, InStream, UndoStream, format, arglist, argfunc, argarray),
                                     trio_pointer_t source, size_t sourceSize,
                                     void(*InStream) TRIO_PROTO((trio_class_t*, int*) ),
                                     void(*UndoStream) TRIO_PROTO((trio_class_t*) ), TRIO_CONST char* format,
                                     va_list arglist, trio_argfunc_t argfunc, trio_pointer_t* argarray)
{
	int status;
	trio_parameter_t parameters[MAX_PARAMETERS];
...
TRIO_PUBLIC int trio_scanfv TRIO_ARGS2((format, args), TRIO_CONST char* format, trio_pointer_t* args)
{
	...
	return TrioScan((trio_pointer_t) stdin, 0, TrioInStreamFile, TrioUndoStreamFile, format, unused, TrioArrayGetter,
	                args);
}
#endif /* TRIO_FEATURE_STDIO */
...
TRIO_PUBLIC int trio_fscanfv TRIO_ARGS3((file, format, args), FILE* file, TRIO_CONST char* format, trio_pointer_t* args)
{
	...
	return TrioScan((trio_pointer_t) file, 0, TrioInStreamFile, TrioUndoStreamFile, format, unused, TrioArrayGetter,
	                args);
}
#endif /* TRIO_FEATURE_FILE */
...
#if TRIO_FEATURE_CLOSURE
TRIO_PUBLIC int trio_vcscanf TRIO_ARGS4((stream, closure, format, args), trio_instream_t stream, trio_pointer_t closure,
                                        TRIO_CONST char* format, va_list args)
{
	trio_custom_t data;
...
#if TRIO_FEATURE_CLOSURE
TRIO_PUBLIC int trio_cscanfv TRIO_ARGS4((stream, closure, format, args), trio_instream_t stream, trio_pointer_t closure,
                                        TRIO_CONST char* format, trio_pointer_t* args)
{
	static va_list unused;
	trio_custom_t data;
...
#if TRIO_FEATURE_CLOSURE && TRIO_FEATURE_ARGFUNC
TRIO_PUBLIC int trio_cscanff TRIO_ARGS5((stream, closure, format, argfunc, context), trio_instream_t stream,
                                        trio_pointer_t closure, TRIO_CONST char* format, trio_argfunc_t argfunc,
                                        trio_pointer_t context)
{
	static va_list unused;
...
/**
   ...
   @return Number of scanned characters.
 */
TRIO_PUBLIC int trio_sscanf TRIO_VARGS3((buffer, format, va_alist), TRIO_CONST char* buffer, TRIO_CONST char* format,
                                        TRIO_VA_DECL)
{
	int status;
	va_list args;
...
/**
   ...
   @return Number of scanned characters.
 */
TRIO_PUBLIC int trio_vsscanf TRIO_ARGS3((buffer, format, args), TRIO_CONST char* buffer, TRIO_CONST char* format,
                                        va_list args)
{
	assert(VALID(buffer));
	assert(VALID(format));
...
/**
   ...
   @return Number of scanned characters.
 */
TRIO_PUBLIC int trio_sscanfv TRIO_ARGS3((buffer, format, args), TRIO_CONST char* buffer, TRIO_CONST char* format,
                                        trio_pointer_t* args)
{
	static va_list unused;
