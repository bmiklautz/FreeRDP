 ...
 */
#if defined(TRIO_FUNC_STRING_CREATE) || defined(TRIO_FUNC_STRING_DUPLICATE) || defined(TRIO_FUNC_XSTRING_DUPLICATE)
 ...
 */
#if defined(TRIO_FUNC_STRING_CREATE) || defined(TRIO_FUNC_STRING_APPEND) || defined(TRIO_FUNC_XSTRING_APPEND) ||       \
 ...
 */
#if defined(TRIO_FUNC_STRING_APPEND) || defined(TRIO_FUNC_XSTRING_APPEND) || defined(TRIO_FUNC_XSTRING_APPEND_MAX)
...
TRIO_PUBLIC_STRING char* trio_create TRIO_ARGS1((size), size_t size) { return (char*) TRIO_MALLOC(size); }
...
TRIO_PUBLIC_STRING size_t trio_length TRIO_ARGS1((string), TRIO_CONST char* string) { return strlen(string); }
...
TRIO_PUBLIC_STRING int trio_equal_case_max TRIO_ARGS3((first, max, second), TRIO_CONST char* first, size_t max,
...
TRIO_PUBLIC_STRING int trio_equal_max TRIO_ARGS3((first, max, second), TRIO_CONST char* first, size_t max,
                                                 TRIO_CONST char* second)
{
	assert(first);
	assert(second);
...
TRIO_PUBLIC_STRING size_t trio_format_date_max TRIO_ARGS4((target, max, format, datetime), char* target, size_t max,
                                                          TRIO_CONST char* format, TRIO_CONST struct tm* datetime)
{
	assert(target);
	assert(format);
	assert(datetime);
...
TRIO_PUBLIC_STRING size_t trio_span_function TRIO_ARGS3((target, source, Function), char* target,
                                                        TRIO_CONST char* source, int(*Function) TRIO_PROTO((int) ))
{
	size_t count = 0;
...
TRIO_PUBLIC_STRING char* trio_substring_max TRIO_ARGS3((string, max, substring), TRIO_CONST char* string, size_t max,
                                                       TRIO_CONST char* substring)
{
	size_t count;
	size_t size;
...
TRIO_PUBLIC_STRING trio_long_double_t trio_to_long_double TRIO_ARGS2((source, endp), TRIO_CONST char* source,
                                                                     char** endp)
{
	...
	if ((source[0] == '0') && ((source[1] == 'x') || (source[1] == 'X')))
	{
		...
		{
			integer *= base;
			integer += (isdigit((int) *source) ? (*source - '0') : 10 + (internal_to_upper((int) *source) - 'A'));
			source++;
		}
		if (*source == '.')
		{
			...
			while (isxdigit((int) *source))
			{
				fracdiv /= base;
				fraction +=
				  fracdiv * (isdigit((int) *source) ? (*source - '0') : 10 + (internal_to_upper((int) *source) - 'A'));
				source++;
			}
...
TRIO_PUBLIC_STRING unsigned long trio_to_unsigned_long TRIO_ARGS3((string, endp, base), TRIO_CONST char* string,
                                                                  char** endp, int base)
{
	assert(string);
	assert((base >= 2) && (base <= 36));
...
TRIO_PUBLIC_STRING int trio_to_upper TRIO_ARGS1((source), int source) { return internal_to_upper(source); }
...
TRIO_PUBLIC_STRING int trio_string_equal_max TRIO_ARGS3((self, max, other), trio_string_t* self, size_t max,
                                                        trio_string_t* other)
{
	assert(self);
	assert(other);
...
TRIO_PUBLIC_STRING int trio_xstring_equal_max TRIO_ARGS3((self, max, other), trio_string_t* self, size_t max,
...
TRIO_PUBLIC_STRING int trio_string_equal_case_max TRIO_ARGS3((self, max, other), trio_string_t* self, size_t max,
                                                             trio_string_t* other)
{
	assert(self);
	assert(other);
...
TRIO_PUBLIC_STRING int trio_xstring_equal_case_max TRIO_ARGS3((self, max, other), trio_string_t* self, size_t max,
                                                              TRIO_CONST char* other)
{
	assert(self);
	assert(other);
...
TRIO_PUBLIC_STRING size_t trio_string_format_date_max TRIO_ARGS4((self, max, format, datetime), trio_string_t* self,
                                                                 size_t max, TRIO_CONST char* format,
