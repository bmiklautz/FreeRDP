...
BOOL Win32_WTSVirtualChannelReadAsync(WTSAPI_CHANNEL* pChannel)
{
	...
	if (pChannel->showProtocol)
	{
		...
		status =
		  ReadFile(pChannel->hFile, pChannel->header, sizeof(CHANNEL_PDU_HEADER), &numBytes, &(pChannel->overlapped));
	}
	else
	{
		status = ReadFile(pChannel->hFile, pChannel->chunk, CHANNEL_CHUNK_LENGTH, &numBytes, &(pChannel->overlapped));
...
BOOL WINAPI Win32_WTSVirtualChannelRead_Static(WTSAPI_CHANNEL* pChannel, DWORD dwMilliseconds, LPVOID lpBuffer,
                                               DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesTransferred)
{
	...
	else if (pChannel->readAsync)
	{
		...
		*lpNumberOfBytesTransferred = 0;
		...
		{
			bSuccess = GetOverlappedResult(pChannel->hFile, &(pChannel->overlapped), &numBytesRead, TRUE);
			...
			*lpNumberOfBytesTransferred += numBytesToRead;
			...
			bSuccess = Win32_WTSVirtualChannelRead_Static(pChannel, dwMilliseconds, lpBuffer, nNumberOfBytesToRead,
			                                              &numBytesRead);
...
BOOL WINAPI Win32_WTSVirtualChannelRead_Dynamic(WTSAPI_CHANNEL* pChannel, DWORD dwMilliseconds, LPVOID lpBuffer,
                                                DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesTransferred)
{
	...
	else if (pChannel->readAsync)
	{
		...
		*lpNumberOfBytesTransferred = 0;
		...
		if (WaitForSingleObject(pChannel->hEvent, dwMilliseconds) != WAIT_TIMEOUT)
		{
			...
			bSuccess = Win32_WTSVirtualChannelRead_Dynamic(pChannel, dwMilliseconds, lpBuffer, nNumberOfBytesToRead,
			                                               &numBytesRead);
                                        ...
                                        DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesTransferred)
{
	...
	else
	{
		if (pChannel->dynamic)
		{
			return Win32_WTSVirtualChannelRead_Dynamic(pChannel, dwMilliseconds, lpBuffer, nNumberOfBytesToRead,
			                                           lpNumberOfBytesTransferred);
		}
		else
		{
			return Win32_WTSVirtualChannelRead_Static(pChannel, dwMilliseconds, lpBuffer, nNumberOfBytesToRead,
			                                          lpNumberOfBytesTransferred);
		}
	}
...
BOOL WINAPI Win32_WTSVirtualChannelWrite(HANDLE hChannel, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite,
...
BOOL WINAPI Win32_WTSVirtualChannelQuery(HANDLE hChannelHandle, WTS_VIRTUAL_CLASS WtsVirtualClass, PVOID* ppBuffer,
...
VOID WINAPI Win32_WTSFreeMemory(PVOID pMemory) { _wts_free(pMemory); }

BOOL WINAPI Win32_WTSFreeMemoryExW(WTS_TYPE_CLASS WTSTypeClass, PVOID pMemory, ULONG NumberOfEntries) { return FALSE; }
