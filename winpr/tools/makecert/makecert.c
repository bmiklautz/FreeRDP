...
static COMMAND_LINE_ARGUMENT_A args[] = {
	...
	{ "format", COMMAND_LINE_VALUE_REQUIRED, "<crt|pem|pfx>", NULL, NULL, -1, NULL, "Specify certificate file format" },
	{ "path", COMMAND_LINE_VALUE_REQUIRED, "<path>", NULL, NULL, -1, NULL, "Specify certificate file output path" },
	{ "p", COMMAND_LINE_VALUE_REQUIRED, "<password>", NULL, NULL, -1, NULL, "Specify certificate export password" },
	...
	{ "pe", COMMAND_LINE_VALUE_FLAG, NULL, NULL, NULL, -1, NULL,
	  "Unsupported - Marks the generated private key as exportable. This allows the private key to be included in the "
	  "certificate." },
	...
	{ "sr", COMMAND_LINE_VALUE_REQUIRED, "<location>", NULL, NULL, -1, NULL,
	  "Unsupported - Specifies the subject's certificate store location. location can be either currentuser (the "
	  "default) or localmachine." },
	...
	{ "$", COMMAND_LINE_VALUE_REQUIRED, "<authority>", NULL, NULL, -1, NULL,
	  ...
	  "(for certificates used by commercial software publishers) or individual (for certificates used by individual "
	  "software publishers)." },
	...
	{ "cy", COMMAND_LINE_VALUE_REQUIRED, "<certType>", NULL, NULL, -1, NULL,
	  "Unsupported - Specifies the certificate type. Valid values are end for end-entity and authority for "
	...
	{ "eku", COMMAND_LINE_VALUE_REQUIRED, "<oid[,oid…]>", NULL, NULL, -1, NULL,
	  "Unsupported - Inserts a list of comma-separated, enhanced key usage object identifiers (OIDs) into the "
	  "certificate." },
	...
	{ "ip", COMMAND_LINE_VALUE_REQUIRED, "<provider>", NULL, NULL, -1, NULL,
	  "Unsupported - Specifies the issuer's CryptoAPI provider name. For information about the CryptoAPI provider "
	  "name, see the –sp option." },
	{ "ir", COMMAND_LINE_VALUE_REQUIRED, "<location>", NULL, NULL, -1, NULL,
	  "Unsupported - Specifies the location of the issuer's certificate store. location can be either currentuser (the "
	  "default) or localmachine." },
	...
	{ "iy", COMMAND_LINE_VALUE_REQUIRED, "<type>", NULL, NULL, -1, NULL,
	  "Unsupported - Specifies the issuer's CryptoAPI provider type. For information about the CryptoAPI provider "
	  "type, see the –sy option." },
	...
	{ "r", COMMAND_LINE_VALUE_FLAG, NULL, NULL, NULL, -1, NULL, "Unsupported - Creates a self-signed certificate." },
	...
	{ "sy", COMMAND_LINE_VALUE_REQUIRED, "<type>", NULL, NULL, -1, NULL,
	  ...
	  "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Cryptography\\Defaults\\Provider Types. If both –sy and –sp are "
	  "present, "
...
static char* x509_get_default_name(void)
{
	...
	if (GetComputerNameExA(ComputerNamePhysicalDnsFullyQualified, NULL, &nSize) || GetLastError() != ERROR_MORE_DATA)
		goto fallback;
...
fallback:
	...
	if (GetComputerNameExA(ComputerNamePhysicalNetBIOS, NULL, &nSize) || GetLastError() != ERROR_MORE_DATA)
		return NULL;
...
static int makecert_context_parse_arguments(MAKECERT_CONTEXT* context, int argc, char** argv)
{
	...
	do
	{
		...
		CommandLineSwitchCase(arg, "live") { context->live = TRUE; }
	...
	if (fp)
	{
		if (context->pfxFormat)
		{
			...
			OPENSSL_init_crypto(OPENSSL_INIT_ADD_ALL_CIPHERS | OPENSSL_INIT_ADD_ALL_DIGESTS | OPENSSL_INIT_LOAD_CONFIG,
			                    NULL);
#endif
			context->pkcs12 = PKCS12_create(context->password, context->default_name, context->pkey, context->x509,
			                                NULL, 0, 0, 0, 0, 0);
